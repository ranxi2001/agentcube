...
func (r *CodeInterpreterReconciler) ensureSandboxWarmPool(ctx context.Context, ci *runtimev1alpha1.CodeInterpreter) error {
	...
	if errors.IsNotFound(err) {
		...
		warmPool = &extensionsv1beta1.SandboxWarmPool{
			...
			Spec: extensionsv1beta1.SandboxWarmPoolSpec{
				Replicas: ptr.To(*ci.Spec.WarmPoolSize),
	...
	needsUpdate := false
	if ptr.Deref(warmPool.Spec.Replicas, int32(1)) != *ci.Spec.WarmPoolSize {
		warmPool.Spec.Replicas = ptr.To(*ci.Spec.WarmPoolSize)
		needsUpdate = true
	}
