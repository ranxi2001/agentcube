...
func TestEnsureSandboxWarmPoolUsesV1Beta1Replicas(t *testing.T) {
	ci := testCodeInterpreterWithWarmPool()
	desired := int32(2)
	ci.Spec.WarmPoolSize = &desired
	reconciler := newTestReconcilerWithObjects()
	...
	require.NotNil(t, warmPool.Spec.Replicas)
	assert.Equal(t, desired, *warmPool.Spec.Replicas)
	assert.Equal(t, ci.Name, warmPool.Spec.TemplateRef.Name)

	updated := int32(3)
	ci.Spec.WarmPoolSize = &updated
	require.NoError(t, reconciler.ensureSandboxWarmPool(context.Background(), ci))
	require.NoError(t, reconciler.Get(context.Background(), types.NamespacedName{
		Name:      ci.Name,
		Namespace: ci.Namespace,
	...
	require.NotNil(t, warmPool.Spec.Replicas)
	assert.Equal(t, updated, *warmPool.Spec.Replicas)
}
