...
When a repeatedly patched, rebased, force-pushed, or squashed PR is presented as ready for final review, reset the review from the parent Issue contract instead of checking only prior findings. Run `/home/agentcube/.agents/skills/agentcube-pr-review/scripts/final_head_review.py` with `--repo-root` pointing to the exact PR worktree and with the Issue/proposal body or explicit acceptance notes. Account for every hand-written changed file and material hunk with a `--scope-closure` bound to the exact base/head/merge-base, map every added or modified `*_test.go` package to an eligible control-flow-free full-package workflow command plus a GitHub-verified exact-head job/step PASS or run it directly, and close every deterministic boundary or external-URL lead. A CI waiver additionally requires immutable setup, exact clean checkout, a concrete trusted GitHub-hosted runner, safe allowlisted flags, no mutable environment/control-flow escape, and an exact-head `push` or `workflow_dispatch` run; `pull_request` synthetic merge-ref runs, filtered/skip/compile/dry-run/zero-count/overlay/unknown-flag commands, injected `GOFLAGS`, custom shells, `continue-on-error`, Makefile/script candidates, static workflow text, and free-form PASS claims are not exact-head proof. Direct fallback requires a clean exact-head input worktree but executes from Git-object-materialized regular files, not ambient/ignored files; it also requires an explicitly reviewed absolute Linux/amd64 `--go-binary`, sanitized environment, `GOTOOLCHAIN=local`, the governing HEAD-tracked `go.work`, all local module paths inside the exact tree, module-aware package mapping, and explicit execution for changed platform/build-constrained tests. Freeze input refs to SHAs before all surface reads. An out-of-scope rationale is not closure: classify the item as `remove` or `separate` and keep review readiness blocked until a new head removes/splits it or a maintainer explicitly changes the merge-unit decision. Validate user-executed external assets and language boundaries such as shell version comparisons. Structural merge conflicts, green check names without mapped PASS evidence, and unresolved leads all block readiness.

For repeated agent workflow optimization, use `/home/agentcube/.agents/skills/agent-autoharness/SKILL.md`. Freeze train/validation/held-out tasks and outcome checks before comparing runs; normalize only observable events, not private chain-of-thought; report task achievement, phase/finding recall, resource efficiency, and trajectory reasonableness separately. Keep unmeasured duration, tokens, and cost as unknown rather than zero, and accept a skill, prompt, tool, memory, or verification change only through paired task-level regression gates. A single post-hoc reconstructed trajectory can validate the evaluator and generate a hypothesis, but cannot establish a population-level improvement.
