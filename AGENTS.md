...
When a repeatedly patched, rebased, force-pushed, or squashed PR is presented as ready for final review, reset the review from the parent Issue contract instead of checking only prior findings. Run `/home/agentcube/.agents/skills/agentcube-pr-review/scripts/final_head_review.py` with `--repo-root` pointing to the exact PR worktree and with the Issue/proposal body or explicit acceptance notes. Account for every hand-written changed file, map every added or modified `*_test.go` package to the exact CI command that executes it, and run any uncovered package directly even when all checks are green. Close every harness row with code/runtime evidence or an explicit out-of-scope rationale. Validate user-executed external assets and language boundaries such as shell version comparisons. Green build, coverage, or E2E names are not proof that a particular changed test was discovered.
