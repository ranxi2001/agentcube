...
For company-facing weekly reports and emails, use `/home/intern-week-mail/.agents/skills/write-weekly-report-email/SKILL.md`, then independently run `/home/intern-week-mail/.agents/skills/review-weekly-report/SKILL.md` against the exact final five-section body. Static validators are regression checks only: final approval requires a line-by-line read and a full top-to-bottom reread. Preserve familiar technical terms such as `E2E`, `CI`, `PR`, `Issue`, and `review`; do not translate them mechanically. Name people by their project role, and rewrite internal shorthand as a concrete cause and practical effect. Any content or status edit after review invalidates the previous `READY` verdict for that section.
