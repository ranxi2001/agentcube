...
Before any upstream-facing action, including creating a PR, draft PR, WIP PR, issue, review comment, issue comment, `/assign`, reviewer request, or maintainer mention, get explicit user confirmation on the exact title/body/comment and target. Do not use upstream PRs as disposable CI runners; validate uncertain fixes in the fork first using fork branches, fork PRs, local tests, or fork CI. Open upstream PRs only when the change is ready for community review or the user explicitly asks to involve upstream. For unfinished upstream work, use the community-style `[WIP]` title prefix, not `[DO NOT MERGE]`, and still use the official PR template.
