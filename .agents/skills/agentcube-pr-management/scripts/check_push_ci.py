#!/usr/bin/env python3
"""Check GitHub Actions push CI runs for an AgentCube fork branch."""
...
DEFAULT_REPO = "ranxi2001/agentcube"
...
def request_json(url: str, token: str | None) -> dict[str, Any]:
    headers = {
        ...
        "User-Agent": "agentcube-push-ci-check",
...
def is_pending(run: dict[str, Any]) -> bool:
    return run.get("status") in PENDING_STATUSES
...
def print_jobs(runs: list[dict[str, Any]], token: str | None, mode: str) -> None:
    if mode == "none":
        return
...
def summarize_runs(
    ...
    print_jobs(runs, token, show_jobs)
    ...
    if failing or unknown_completed:
