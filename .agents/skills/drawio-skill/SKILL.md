...
| Edge-label overlap | Text on long edges overlaps another label, line, or node in the exported PNG | Keep the label on the edge, but add white label background and move it locally with edge geometry `x`/`y` offsets into adjacent whitespace |
