...
- Community freshness loop：用户要求主动发现新 issue/PR 进展，不再等待人工转述。最近一次扫描为 `2026-07-17 09:16 CST`；agent-sandbox v0.5.2 已发布，#438 release blocker 已解除并完成一次用户确认的提醒；#434 仍有同作者 open PR #435。当前无可无冲突认领的新 issue。
...
- agent-sandbox v0.5.2 independent adapter：用户决定在 personal fork 做一份 blind implementation，用于未来对照 #438 作者方案与快速 review。必须从最新 `upstream/main` 建干净 worktree，只使用官方 v0.5.2 API/migration guide 与本地既有证据，不读取未来作者分支；先覆盖 compile/API、storedVersions upgrade、direct/warm claim adoption、delete/GC 和 pool refill。只允许 push fork topic branch，不创建 upstream PR 或竞争 ownership。
...
- Community tasks：每个 substantive work loop 开始先按 last-scan timestamp 刷新新建/更新 issue，并交叉 assignee、`/assign`、same-topic PR 和 maintainer blocker。#438 已解除 release blocker并提醒一次，下一步只等 assignee PR；#434 已有 #435。不竞争实现或重复催促。
- For independent v0.5.2 work：创建 clean fork-only topic branch并保留盲实现边界；先建立 v0.4.6 -> v0.5.2 API/storage/lifecycle rationale matrix，再实现和分层验证。等 #438 作者 PR 出现后才做 diff-to-diff comparison；比较前冻结我们自己的 commit/test record。
