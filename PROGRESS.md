...
- Day44 / PR #431：正式 proposal PR `https://github.com/volcano-sh/agentcube/pull/431`，latest observed head `35d361e` (`fix stale state issue`)，title `[Proposal] add sandbox-pool management proposal`，关联 discussion #430。PR body 已从 `Fixes #430` 改为 `Refs #430`，front matter 已补 `tracking-issue: "#430"`，DCO success；新 commit 后 build/codegen/lint/python/coverage/e2e 等普通 checks 已绿，tide 仍因缺 `approved`/`lgtm` pending。#430 仍 open。无需回复已解决的 scope 线程。已发一条 inline clarification comment 到 line 604：placeholder-agent stale/unreachable status semantics，URL `https://github.com/volcano-sh/agentcube/pull/431#discussion_r3549854078`；作者回复 `Good suggestion. Updated related content` 并通过 `35d361e` 基本吸收，新增 controller-owned `PlaceholderAgentHealthy` 和 `NodeCtl.LastHeartbeat > 2min` stale detection。
- Day44 输出：`internship-reports/day44-sandbox-pool-management-proposal-review.md` 已补 proposal review 方法论、#430 原始讨论问题拆分、#431 架构剖析、从未来开发 owner 视角整理的待问问题、Mermaid 图、GPT image 视觉版 `day44-sandboxpool-gpt-image-architecture.png`、可编辑 draw.io 源 `day44-sandboxpool-architecture.drawio`。`internship-reports/day44-sandboxpool-pr431-comment-drafts.md` 已整理 5 个候选 upstream comment 的证据、判断和英文草稿，推荐只发 stale/unreachable placeholder-agent semantics 这一条。核心理解：#431 不是完整 session lifecycle，而是 slow resource track 的 K8s-native capacity/control-plane；对象模型是 `SandboxPoolClass` 全局策略、per-node `SandboxPool` policy snapshot/status、Static Pod/mirror Pod 资源锁、host-level `placeholder-agent`、黑盒 `node-ctl`。
- #431 后续观察点：二次精读确认 proposal 已回答 Implementation Plan、v1alpha1 scope、Non-Goals、component responsibility、phase/conditions、creation/update/deletion flow、RBAC/webhook/version/test plan 的大框架；不要重复问这些。最新真人 MEMBER 评论来自 `@acsoto`，询问新 SandboxPool 与现有 `CodeInterpreter.warmPoolSize` / `SandboxTemplate` / `SandboxWarmPool` / `SandboxClaim` 路径是并存、下层化还是替代关系；作者回复称二者是 two-generation architecture，SandboxPool 是 new fast-resource architecture 的 slow resource prerequisite，SandboxTemplate/SandboxClaim 属于 fast resource system 相关内容，但正文似乎还没补 Relationship section。Copilot comments 已覆盖 `<5s` mirror pod rebuild、`pause:3.9` vs no-process/no-cgroup、SSA conditions list-map schema、node-ctl endpoint source of truth；最新又新增 3 条 `omitempty` on non-pointer struct fields（placeholderPodTemplate、nodeCtl、status sub-structures）评论。我们已发 stale/unreachable inline comment，`35d361e` 已基本吸收，暂不追加同类评论；可观察 Phase table 中 `PlaceholderAgentHealthy=True → Ready` 是否过宽。不承诺实现。
