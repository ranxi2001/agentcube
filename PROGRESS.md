...
- Community freshness scan：最近一次增量复核到 `2026-07-31 10:15 CST`。`09:31 CST` 后没有新的 issue、PR push 或 maintainer response；#446 只新增了我们已确认发布的 final-head review，其他候选无新变化。
...
- #446 `Upgrade agent sandbox v0.5.3`：open、non-draft、current head `449fb75`，11 commits、35 files、`MERGEABLE` / `UNSTABLE`。11 项实际 Actions checks 全绿；DCO 因后续 10 commits 缺 signoff 失败，Tide 等 `lgtm/approved`。`2026-07-31 10:15 CST` 已对 exact head 发布 [COMMENT review](https://github.com/volcano-sh/agentcube/pull/446#pullrequestreview-4824881370)，三个 fresh unresolved threads 分别要求 active/warm v1alpha1 claim 保持原 binding、移除无关的 Router h2c 默认关闭、修复 Docusaurus mirror helper 404。旧 production scheme thread 仍是 bookkeeping active；不自动 resolve、`/lgtm` 或 mention maintainer。
...
- #446：`449fb75` final-head review 已完成并发布，当前 NOT READY。等待作者补 active/warm alpha claim identity fixture、移除 Router h2c scope drift、同步 Docusaurus helper URL、修复 10 个 unsigned commits；新 push 后重新运行 final-head ledger 和 targeted tests，不把 resolved thread 当修复证据，也不把 fork adapter 开成竞争 PR。
