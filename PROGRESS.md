...
- Community freshness scan：最近一次增量复核到 `2026-08-03 11:35 CST`。#446 current exact head `d7333cc`；作者在我们的 webhook timeout thread 回复 `done.`，新增 commit 只在第 30 次失败诊断后加入 `exit 1`。全部 executable checks 通过；DCO 仍因 `da4140d` / `353f1df` 无 signoff 失败。#450 exact head `f722b51` 的 executable checks/DCO 全绿，Tide 只等 `lgtm/approved`。
...
- #446：`d7333cc` 已关闭我们最新的 webhook fail-open finding；不需要追加 “thanks” 或重复评论，也不自动 resolve、`/lgtm`、mention maintainer。继续等待作者修复 DCO，并由现有公开线程/maintainer 决定 migrated lifecycle、`workloadRef` 与 docs webhook；scheme-test CI discovery 留作独立仓库级后续。
