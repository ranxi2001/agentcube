...
- Community freshness scan：最近一次增量复核到 `2026-07-30 17:13 CST`。`upstream/main` 仍为合入 #448 的 `0704bb9`；#446 已 force-push/squash 为 `2eefda6`，吸收 #448 后删除旧 MCP 冲突和无效 upgrade fixture，11 项实际 checks 全部通过，仅 Tide 等待 `lgtm` / `approved`。#435 等其他线程没有改变当前选择。
...
- #446 `Upgrade agent sandbox v0.5.3`：open、non-draft、current head `2eefda6`，单个 signed commit、33 files、`MERGEABLE` / `UNSTABLE`。11 项实际 checks 与 DCO 全绿，仅 Tide 等待 `lgtm` / `approved`；旧 MCP conflict 和无效 upgrade fixture 已修，不得继续引用 `c0fc500` 的 CI failure。production scheme finding [inline 3671892415](https://github.com/volcano-sh/agentcube/pull/446#discussion_r3671892415) 已 code-addressed。仍有两项 focused finding：`TestSandboxVolumeClaimTemplatesImmutability` 只断言内存 struct，无法执行 v0.5.3 CEL；`hack/update-codegen.sh` 仍含个人 Windows PATH、GNU-only source patch 和整目录重生成。任何公开 review 仍需用户确认 exact target/body/event。
