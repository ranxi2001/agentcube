...
- Community freshness scan：最近一次全量只读扫描为 `2026-07-29 15:10 CST`；`upstream/main` 仍为 `87e6e37`（merged #445 GitHub Actions pins）。`16:52 CST` targeted CI scan 确认 #435、#429、#446 的新 E2E 均解析到 `mcp 2.0.0` 并出现同类 MCP 启动/rollout 失败。用户确认后已创建/edit shared bug [#447](https://github.com/volcano-sh/agentcube/issues/447)，明确同一官方 `mcp` SDK 内的 v1 -> v2 API migration；maintainer `@RainbowMango` 已选择完整 v2 方案。clean branch `fix/mcp-python-sdk-v2@1286b3a` 已 push fork，9/9 workflow success，含 Python 3.11 + Kind 的 local HTTP、stdio、Docker rollout 与 in-cluster MCP E2E；尚未创建 upstream PR，等待用户确认 exact draft。
...
- #447 `Code Interpreter MCP integration fails with mcp 2.0`：open、`kind/bug`、assignee `ranxi2001`；maintainer 明确要求采用 latest v2 SDK。fork branch `fix/mcp-python-sdk-v2@1286b3a` 基于 `upstream/main@87e6e37`，7 files `+55/-45`，DCO signoff；fork exact-head 9/9 workflow success，run `30444105301` 中 `mcp==2.0.0` 的 HTTP/stdio/in-cluster MCP 全通过。PR draft title `fix: migrate Code Interpreter MCP to SDK v2`，body 194 words / 15 nonblank lines；未获 exact confirmation 前不要创建 upstream PR。
- #446 `Upgrade agent sandbox v0.5.3`：open、non-draft、作者频繁补丁后的 current head 为 `83002f1`。production alpha scheme/watch 与 beta reconcilers/builders mismatch 已用三类型 red test + 最小 wiring green tests 证明。用户确认后已发布 [review 4805118919](https://github.com/volcano-sh/agentcube/pull/446#pullrequestreview-4805118919) / [inline 3671892415](https://github.com/volcano-sh/agentcube/pull/446#discussion_r3671892415)，绑定当时 right-side `pkg/workloadmanager/sandbox_controller.go:46`，103 words。latest E2E run `30434433579` 也命中 shared MCP 2.0 drift；这不排除 #446 自身另有回归。继续等待作者稳定 head，不追评、不 `/lgtm`。
...
- #446：等待作者对 inline 3671892415 的回复或新 head；没有新证据不追评。
- #447：向用户展示 exact upstream PR target/title/body、7-file diff 和 tests；仅在确认后创建 PR。PR 创建后读取 bot/CI/reviewer 状态，不自动 request review 或发 comment。
