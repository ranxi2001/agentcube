...
- Community freshness loop：最近一次只读扫描冻结于 `2026-07-21 14:45 CST`，详见 Day52。新增 #444 Router -> PicoD mTLS issue，当前无 assignee、`/assign`、评论或 PR；它与 #442 migration scope 无关。PR #400 也有更新；#438 assignee 和 same-topic PR ownership 未变。#441/#444 均需先厘清 auth/authz、transport 与 execution identity，不抢实现。
...
- Community tasks：每个 substantive work loop 开始以 `2026-07-21 14:45 CST` 为 last scan 刷新新建/更新 issue，并交叉 assignee、`/assign`、same-topic PR 和 maintainer blocker。#442 等待新 head；不因作者 `Ok` 或旧 SHA 绿 CI 提前 resolve。新 #444 暂无 owner，但先做 auth/transport contract 分析而非立即认领；之后再 review #412、本地梳理 #441/#444 identity/permission matrix。
