...
- Community freshness scan：最近一次增量复核到 `2026-07-30 11:50 CST`。`upstream/main` 仍为合入 #448 的 `0704bb9`；#446 通过 merge commit `c0fc500` 合入新 `main`，但 conflict resolution 把 MCP v2 的 `httpx2` 与旧 `sse_client` import、未定义 `streamable_http_client` 混用。current Python Lint 和两个 E2E jobs 均失败，DCO action-required；#435 等其他线程没有改变当前选择。
...
- #446 `Upgrade agent sandbox v0.5.3`：open、non-draft、current head `c0fc500`，7 commits、36 files、merge state `UNSTABLE`。production alpha/beta scheme finding [inline 3671892415](https://github.com/volcano-sh/agentcube/pull/446#discussion_r3671892415) 已由 `27517d0` code-addressed但 thread 未 resolve；用户此前确认并已发布 [55-word reply](https://github.com/volcano-sh/agentcube/pull/446#discussion_r3676642917) 与 [merge notice + `/retest`](https://github.com/volcano-sh/agentcube/pull/446#issuecomment-5125559024)。作者没有 rebase/drop overlap，而是 merge `main`；current merge resolution 把 #448 的 MCP client import/transport 修坏，Python Lint run `30511568113` 观察到 2 个 F401 和 2 个 F821，E2E run `30511568062` 观察到 local MCP 不监听端口和 in-cluster deployment rollout timeout。其 `TestSandboxVolumeClaimTemplatesImmutability` 只断言内存 struct，删除 v0.5.3 CEL 规则后仍会通过；`hack/update-codegen.sh` 仍含个人 Windows PATH 和整目录重生成。等待 head 稳定后再决定是否起草 focused review，任何发布仍需用户 exact confirmation。
...
- #446：先等 `c0fc500` 之后的新补丁；复核时优先检查 MCP merge、无因果的 immutability test、非可移植 codegen 脚本和 upgrade fixture。不要把 fork adapter 直接开成竞争 PR。
