...
- Weekly report workflow：`write-weekly-report-email` 已接入独立 `review-weekly-report`，最终放行必须逐句通读五个板块并返回 `READY`，静态 validator 只能作为回归证据。2026-07-20 用户复核发现上一版仍把 `Feature Proposal Review`、`PR Review`、`inline comments` 和 agent-sandbox 适配语境机械翻译，并用“CI 绿灯”代替“CI 通过”；这证明通顺不等于术语正确。skills/AGENTS/validator 已新增 artifact name + work type gate，Week6 全文修正后 57 tests、strict validation、layout 与敏感信息检查通过，commit `de95e6d` 已推送公开周报仓库供领导审查。邮件未授权发送。
