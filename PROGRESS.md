...
- Community freshness loop：最近一次只读扫描冻结于 `2026-07-21 16:05 CST`。14:45 后没有新的 issue/PR update；#444 Router -> PicoD mTLS 仍无 assignee、`/assign`、评论或 PR。#437 已进入只读 review prep，不涉及 issue ownership。
...
- Community tasks：每个 substantive work loop 开始以 `2026-07-21 16:05 CST` 为 last scan 刷新新建/更新 issue，并交叉 assignee、`/assign`、same-topic PR 和 maintainer blocker。#442 等待新 head；#437 下一步是准备 SIGTERM inline 并让用户确认，不自动发布；#444 先做 auth/transport contract 分析而非立即认领。
