...
- Day44 / PR #431：正式 proposal PR `https://github.com/volcano-sh/agentcube/pull/431`，latest observed head `9f03cca`，title `[Proposal] add sandbox-pool management proposal`，关联 discussion #430。PR body 已从 `Fixes #430` 改为 `Refs #430`，front matter 已补 `tracking-issue: "#430"`，DCO success，#430 仍 open。无需回复已解决的 scope 线程。
- Day44 输出：`internship-reports/day44-sandbox-pool-management-proposal-review.md` 已补 proposal review 方法论、#431 架构剖析、Mermaid 图、GPT image 视觉版 `day44-sandboxpool-gpt-image-architecture.png`、可编辑 draw.io 源 `day44-sandboxpool-architecture.drawio`。核心理解：#431 不是完整 session lifecycle，而是 slow resource track 的 K8s-native capacity/control-plane；对象模型是 `SandboxPoolClass` 全局策略、per-node `SandboxPool` policy snapshot/status、Static Pod/mirror Pod 资源锁、host-level `placeholder-agent`、黑盒 `node-ctl`。
- #431 后续观察点：先等 e2e/tide 和真人 maintainer review。若用户确认，再考虑单点英文评论：Static Pod + skip-cgroup + RuntimeClass CRI handler 是否能稳定做资源锁、Static Pod manifest 与 InPlaceResize 是否真能原地 resize、placeholder-agent stale heartbeat 是否会误报 Ready、`sandbox-pool.io` API group 与 node-ctl endpoint source of truth 是否清晰。不承诺实现。
