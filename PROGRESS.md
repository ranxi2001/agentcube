...
- Community freshness loop：最近一次只读扫描冻结于 `2026-07-17 18:06 CST`，详见 Day51。新增高信号是 #441 要求先厘清 auth/authz、Kubernetes execution identity 与 namespace/session ownership，当前不适合实现；#412 latest `2aa1831` checks 全绿但新 push 移除旧 `lgtm`，与 v0.5.2 adapter 重叠 10 文件、与 Pod cleanup 重叠 4 文件，只做 review。#438 仍由 `@safiya2610` 负责且无 PR，#439 checks 绿但无真人 review。当前无可无冲突认领的新 issue。
...
- #438 agent-sandbox v0.5.2+：v0.5.2 已于 2026-07-17 发布并包含 #1124；用户确认后已 [提醒 assignee](https://github.com/volcano-sh/agentcube/issues/438#issuecomment-4998039228)。当前无 linked PR；不重复催促或实现，等待作者 PR 后做 migration/lifecycle/E2E review。
...
- #385 WarmPoolAvailable PoC：exact head `d885b4e` 相对 `main@146b75f` 为 `56 behind / 3 ahead`，当前 `DIRTY`；#387 在 CodeInterpreter controller/test 触发冲突。Day51 确认需求仍存在且 v0.5.2 不会自动补父资源 health projection，但旧 head 还漏了 WorkloadManager core Events RBAC，现有 recovery test 也未证明真实 `Owns` watch。不要催 review 或直接 rebase；等 #438 beta 基线后重审 `Ready`/50% threshold/freshness、RBAC 和非跳过 E2E，再由用户确认是否更新现有 PR。
...
- Community tasks：每个 substantive work loop 开始先按 `2026-07-17 18:06 CST` 刷新新建/更新 issue，并交叉 assignee、`/assign`、same-topic PR 和 maintainer blocker。当前顺序为 review #412 稳定 head、本地梳理 #441 identity/permission matrix、等待 #438 assignee PR 后做 diff-to-diff；不竞争实现、重复催促或自动发评论。
- For independent v0.5.2 work：保持 fork head `2d90b07` 和 Day50 证据不变；等 #438 作者 PR 出现后先冻结其 exact head，再按 API strategy、WarmPoolRef、pointer replicas、Store identity、migration 和真实 E2E 做 diff-to-diff review。原地升级另在独占 runner 验证，不把 clean install 当 migration 证据。
