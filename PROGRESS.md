...
- #387 agent-sandbox v0.4.6 compatibility：current head `df8eb9b`，`mergeable=true`/`rebaseable=true`；test-only 双 job 与 Ubuntu 24.04 runner 已进入 PR，latest official 12/12 checks 全绿。Mermaid reviewer note 已发布；下一步等待 `lgtm`/`approved`，不自动追加评论或请求 review。
...
- For #387: test-only update、Ubuntu 24.04 runner 和 Mermaid reviewer note 均已完成。下一步只等待 maintainer review/labels；不要追加 #433 auth/RBAC 修改，也不要自动继续评论或请求 review。
