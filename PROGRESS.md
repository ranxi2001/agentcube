...
- Day48 post-merge / OWNERS：Claim polling 当前不改 watch。Pod fallback cleanup 已在 branch `cleanup/remove-sandbox-pod-fallback` 形成 DCO commit `eefce59`（6 files `+45/-231`），unit/race/repeat/lint/qualified-Helm 通过；它移除全集群 Pod informer/cache-sync 与 `list/watch` RBAC，不称 #387 bug。`RainbowMango` 提名已按 issue/comment/review/outcome 重做依据；branch `owners/add-rainbowmango` DCO commit `63bea7a` 仅改 root `OWNERS` 两行。两者均未 push/open PR。Volcano sponsor/tenure 由现有 root approver 确认。
...
- For RainbowMango OWNERS nomination：等待用户确认 exact target `volcano-sh/agentcube`、title、224-word body、两行 diff 和 YAML/order/DCO validation；确认后只 push `origin owners/add-rainbowmango` 并创建 nomination PR。不得把 Day48/中文报告带入 topic branch，也不得伪造 sponsor、2FA 或任职年限。
- For Pod informer cleanup：等待用户单独确认 title、208-word body、6-file `+45/-231` diff、unit/race/repeat/lint/qualified-Helm evidence；确认后只 push `origin cleanup/remove-sandbox-pod-fallback` 并创建独立 PR。默认 PATH 无 Helm，必须准确写已有 `v3.18.4` binary 的 PATH-qualified 验证。
