...
- Community freshness scan：最近一次只读复核到 `2026-08-31 12:35 CST`。上次扫描后只有 #429 更新：该 PR 仍 open/non-draft、mergeable/unstable，head `8bfb7bf0`，1 commit、2 files、`+407/-0`；普通 E2E 与其余常规 checks 通过，CodeInterpreter E2E 失败，Tide pending。`upstream/main` 仍为 `7a85d4f7`（Go 1.26.7）。保持现有 stop conditions，不自动发布 upstream 内容。
...
- Day59 AgentENV 调研：固定 `kvcache-ai/AgentENV@0475f403` 后确认，它不是“完全脱离 Kubernetes”，而是 sandbox runtime / lifecycle / hot path Kubernetes-independent，同时保留第一方可选的 Kustomize 部署和 `client-go` EndpointSlice / Pod discovery。每个 sandbox 是 node-local Firecracker backend，不创建 Pod/CRD；scheduler binding 默认内存、可选 Redis，lifecycle truth 在 Rust orchestrator。`(cd services && go test ./...)` 与 `make k8s-render` 通过；本轮研究容器无 `cargo`、`/dev/kvm`，未复现 README 性能数字。完整证据见 `internship-reports/day59-kvcache-ai-agentenv-kubernetes-boundary-study.md`。
- Day60 Volcano / Kthena 调研：固定 `volcano@6575302c` 与 `kthena@1b3319d0`。Volcano 是 Kubernetes 内的 Job/PodGroup/Queue 与 `Pod -> Node` placement plane；Kthena 是 LLM lifecycle + `Request -> Backend Pod` routing plane。Kthena chart 不安装 Volcano，但 API 直接依赖 Volcano、ModelServing 默认 `schedulerName=volcano`，改用 kube-scheduler 会失去 gang/queue/topology；current docs 还残留已删除的 AutoscalingPolicyBinding，Router readiness、Redis Secret key 和部分 API 接线也有固定 SHA 漂移。两边聚焦 Go tests 与 Helm lint/template 通过；Kthena 非 E2E Go packages 通过，Volcano exact-head HyperNode E2E 仍失败；本轮无 active cluster / GPU E2E。完整证据见 `internship-reports/day60-volcano-kthena-architecture-and-project-study.md`。
...
- #429 Go toolchain update workflow：open、remote head `8bfb7bf0`，1 commit、2 files、`+407/-0`，基于当前 `upstream/main@7a85d4f7`；2026-08-31 exact-head 普通 E2E 与其余常规 checks 通过，CodeInterpreter E2E 失败，Tide pending。下一步先定位该 exact-head 失败是否与两文件 scope 相关，再决定是否需要本地修正；不直接催 reviewer 或自动更新公开分支。
...
- #429：保持 `8bfb7bf0` 两文件 scope；先定位 exact-head CodeInterpreter E2E failure 是否与当前 diff 有关，再让用户确认任何 open PR branch update。
...
- Volcano / Kthena：后续 review 用三层调度口径，不能把 Kthena `Request -> Backend Pod`、Volcano `Pod -> Node` 和 AgentENV `Sandbox -> Runtime Node` 混为一种 scheduler。若转成 AgentCube design，先定义 request routing、lifecycle、cluster placement、node-local fast plane 的 state owner 与 failure contract，再决定是否引入 Volcano 或下沉 hot state。
