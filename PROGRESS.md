...
- Final-head review harness：`agentcube-pr-review/scripts/final_head_review.py` 现支持 versioned `--finding-ledger` + exact-head `--finding-closure`，绑定 ledger ID/version/content digest、current PR 和 exact head；decision status 需要 current-PR comment/maintainer association，`--run-go-tests` 拒绝 dirty worktree。#446 15 项 technical ledger 为 13 fixed / 2 present / 0 unclassified；结构 closure complete，但 readiness blocked / exit 1。
- Review harness forward replay：独立回放确认 current #446 只剩上述两项，也抓到 mTLS false positive；`review_surface.py` 现把 env 绑定到实际运行 E2E 的 job/step，解析该 job 的 matrix include/block list/exclude 与 inline assignment，不能跨 job/step 拼接证据。closure/digest/readiness/schema-type bypass 不能通过；finding mode 必须显式选择 versioned ledger 或 `--no-carry-forward-findings`，结果区分 `none-declared` 与漏传的 `not-provided`；PR Review scripts 40 tests pass。这仍是同 lineage regression，不是 held-out promotion。
