...
- #431 后续观察点：二次精读确认 proposal 已回答 Implementation Plan、v1alpha1 scope、Non-Goals、component responsibility、phase/conditions、creation/update/deletion flow、RBAC/webhook/version/test plan 的大框架；不要重复问这些。最新真人 MEMBER 评论来自 `@acsoto`，询问新 SandboxPool 与现有 `CodeInterpreter.warmPoolSize` / `SandboxTemplate` / `SandboxWarmPool` / `SandboxClaim` 路径是并存、下层化还是替代关系；这是当前最高权重问题，先等作者回应。Copilot comments 已覆盖 `<5s` mirror pod rebuild、`pause:3.9` vs no-process/no-cgroup、SSA conditions list-map schema、node-ctl endpoint source of truth。我们已发 stale/unreachable inline comment，后续等作者回应，不要继续追加同类评论。不承诺实现。
