...
- Community freshness scan：最近一次增量复核到 `2026-07-30 10:07 CST`。`upstream/main` 已通过 merge commit `0704bb9` 合入 #448，关联 bug #447 随即关闭；#446 当前回到 head `83002f1`，仍保留 SSE workaround，且相对新 `main` 为 `mergeable_state=dirty`。#435 没有新代码提交，维护者 @acsoto 新增评论 `e2e breaks`；其余扫描结果没有改变当前选择。
