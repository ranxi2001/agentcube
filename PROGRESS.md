...
- #438 / #442 agent-sandbox v0.5.2+：current head `c9e0498`，2 commits、27 files、12 checks 绿；第二个 commit 是 merge main，触发 `do-not-merge/contains-merge-commits`。作者已 [acknowledge review](https://github.com/volcano-sh/agentcube/pull/442#issuecomment-5018460089)，但 migration docs、upgrade E2E、`doc.go`/full GVR 三处均未改，三个 human threads 仍 current/unresolved。等待 fix + rebase，不自动回复。
