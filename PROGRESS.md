...
- For #431: 当前固定 head `f380208`；不新增评论。等待 8 个 active thread 收敛；新 push 后先复核单一 Selector、ResourceList/percentage 模型、template propagation、NodeCtlEndpoint 删除、逐字段 API comments 和 RuntimeClass bootstrap。任何 upstream 回复仍需用户确认 exact body。
