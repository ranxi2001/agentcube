...
- Day20 后续：如继续做架构学习，下一步专项深读 Python SDK / CLI auth/session 封装，或补 agent-sandbox controller 源码状态机。0.2/0.3 分段适配实验分支已本地提交：`/home/agentcube-agent-sandbox-v02` head `a4506d6`，`/home/agentcube-agent-sandbox-v03` head `44f473c`；尚未 push。不要把 Day20 的 general notes 混入 upstream PR。
