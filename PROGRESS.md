...
- Community freshness scan：最近一次增量复核到 `2026-08-07 11:14 CST`。#446 exact head 仍为 `7a17302`，current `main` 仍为 `939abb5`；没有新的 upstream commit、Actions run、review 或 accepted-risk 决定。status rollup 只有 DCO `action_required` 与 Tide `pending`；DCO 仍因 `da4140d` / `353f1df` 无 signoff 失败，history 仍含 merge commit `3882157`，`do-not-merge/contains-merge-commits` 未移除。
