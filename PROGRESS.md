...
- #446：结论仍为 **NO `/lgtm`**。若用户要发布，准备一个绑定 `8a528f1` 的 concise `COMMENT` review：明确 F19 已 fixed，只反馈 F09 的 exact producer mismatch，即手工 bound Sandbox 转换到 `warmPoolRef=upgrade-bound`，而断言观察的是另一个 `e2e-upgrade-warmpool` 原本就 ready 的 member；不要重复旧措辞。发布前重新核对 exact head 并让用户确认全文/event。作者还需 rebase/squash merge commit 并修复两项 DCO；F21 不扩进本 PR blocker comment。
