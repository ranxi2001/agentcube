...
- #431 后续观察点：`@acsoto` 的 WarmPool relationship 问题仍未进入正文。当前未被既有评论覆盖的 P1 是：`SP-11` heartbeat 超时缺 `RequeueAfter`/periodic sweep、`SP-12` per-node single-Class 跨对象 webhook check 非原子、`SP-05` force-finalizer 后 orphan manifest。三项已有本地草稿，一次最多考虑发一条且必须先让用户确认 exact target/body。`SP-10` no-process shim 仍缺完整 Task PID/Wait/exit contract，保留为 Phase 2 spike/e2e gate；containerd main `ba01536` proto 已本地复核。
