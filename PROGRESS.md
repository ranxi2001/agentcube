...
- #446 `Upgrade agent sandbox v0.5.3`：open、non-draft、current `a0f4882`，1 signed commit、35 files；全部 executable checks/DCO 通过，Tide 等 `lgtm/approved`。code-generator skew 已修；20-item closure 为 14 fixed / 6 present。F09 已公开且不要重复；Store OwnerID、GOBIN、PodSpec `workloadRef` 与 webhook docs 是新 finding，scheme test CI discovery 是既有 coverage residual。本轮未发布 upstream 内容。
