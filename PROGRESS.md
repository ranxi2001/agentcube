...
- Day44 / PR #431：正式 proposal PR `https://github.com/volcano-sh/agentcube/pull/431`，latest observed head `35d361e` (`fix stale state issue`)，title `[Proposal] add sandbox-pool management proposal`，关联 discussion #430。PR body 已从 `Fixes #430` 改为 `Refs #430`，front matter 已补 `tracking-issue: "#430"`，DCO success；新 commit 后 build/codegen/lint/python 等已绿，coverage/e2e/tide 仍 pending。#430 仍 open。无需回复已解决的 scope 线程。已发一条 inline clarification comment 到 line 604：placeholder-agent stale/unreachable status semantics，URL `https://github.com/volcano-sh/agentcube/pull/431#discussion_r3549854078`；作者未直接回复但 `35d361e` 已基本吸收，新增 controller-owned `PlaceholderAgentHealthy` 和 `NodeCtl.LastHeartbeat > 2min` stale detection。
