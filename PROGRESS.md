...
- Day44 输出：`internship-reports/day44-sandbox-pool-management-proposal-review.md` 保存完整架构剖析、图和 review 方法；`internship-reports/day44-sandboxpool-pr431-comment-drafts.md` 已按 head `35d361e` 继续审阅。当前 P0 是 Static Pod 与 Kubernetes native in-place resize 不兼容：mirror Pod 不能从 API 控制；v1.36.2 默认按完整 manifest 内容生成 Static Pod UID，资源变化会产生新 UID并走 replacement，而非 same-Pod resize。#431 不是整体不可用，冲突集中在 Phase 3 / v1alpha1 no-rebuild resize；Class→Pool、固定容量 reservation、status/phase 仍可独立实现。P1 是 RuntimeClass 到独立 `placeholder-agent` socket 的集成层缺失。
