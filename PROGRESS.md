...
- Day48 post-merge / OWNERS：Pod cleanup branch `cleanup/remove-sandbox-pod-fallback@eefce59` 与 RainbowMango nomination branch `owners/add-rainbowmango@63bea7a` 已 push 到 fork，upstream PR 均未创建。后续历史审计确认 AgentCube #137 是手工绕过 invalid-owners 的 bootstrap 特例；近期可复用路径是 community membership issue 承载候选人确认、sponsor 和贡献证据，再分别提交 Reviewer、至少 2 个月后提交 Approver。当前两角色 branch 仅供检查，不应直接建 PR，除非现有 maintainers 明确确认 AgentCube 独立例外。
...
- For RainbowMango OWNERS nomination：fork branch `owners/add-rainbowmango@63bea7a` 已 push，但 direct reviewer+approver PR 处于 `HOLD_GOVERNANCE`。220-word body 是可复用贡献证据账本，不再视为可直接发布的 final PR body。优先让候选人按 community template 确认 membership/sponsors 并先申请 Reviewer；或者由现有 maintainers 明确说明 AgentCube 是否采用独立例外。不得代填 2FA、sponsor consent 或 tenure。
