...
- Weekly report workflow：`write-weekly-report-email` 已接入独立 `review-weekly-report`，最终放行必须逐句通读五个板块并返回 `READY`，静态 validator 只能作为回归证据。2026-07-20 用户复核先后纠正机械翻译、社区交付状态和裸 PR 编号三类问题：保留 `Feature Proposal Review` / `PR Review` / `inline comments`，CI 写“通过/失败”；本人完成状态不依赖维护者时间；每个 `#number` 必须在同一行带仓库和具体主题。skills/AGENTS/validator 已新增 artifact/work type、status ownership 与 `E215` reference-topic gates；Week6 HTML commits `de95e6d`、`aff75c3`、`0fd1e97` 均已推送。59 tests、strict validation、layout 与敏感信息检查通过；邮件未授权发送。
