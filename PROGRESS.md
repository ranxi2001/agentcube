...
- #446：current `353f1df` 已完成 exact-head 21-item closure；16 fixed、5 present。待用户确认 exact payload 后，发布 1 个 `COMMENT` review：`test/e2e/run_e2e.sh:452` 指出 webhook probe exhaustion 仍继续 mutation，`cmd/workload-manager/main_test.go:29` 指出 scheme regression test 未被 PR checks 执行。发布前再 fail-closed 核对 head；旧 findings 不重复发布，不自动 resolve、`/lgtm`、mention maintainer，也不把 fork adapter 开成竞争 PR。
