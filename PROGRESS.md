...
- Day44 输出：`internship-reports/day44-sandbox-pool-management-proposal-review.md` 已补 proposal review 方法论、#430 原始讨论问题拆分、#431 架构剖析、Mermaid 图、GPT image 视觉版 `day44-sandboxpool-gpt-image-architecture.png`、可编辑 draw.io 源 `day44-sandboxpool-architecture.drawio`。核心理解：#431 不是完整 session lifecycle，而是 slow resource track 的 K8s-native capacity/control-plane；对象模型是 `SandboxPoolClass` 全局策略、per-node `SandboxPool` policy snapshot/status、Static Pod/mirror Pod 资源锁、host-level `placeholder-agent`、黑盒 `node-ctl`。
