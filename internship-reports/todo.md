...
| P0 | 适配 current stable `kubernetes-sigs/agent-sandbox` | REVIEW | #386；PR [#387](https://github.com/volcano-sh/agentcube/pull/387)；Go 前置 PR [#391](https://github.com/volcano-sh/agentcube/pull/391) 已合并 | 高 | 低 | 1-2 天 | [Day 16](day16-agent-sandbox-latest-adaptation.md) 保存 v0.4.6 适配与 runtime 验证；[Day 17](day17-pr387-copilot-review-and-ci-triage.md) 保存 review/CI triage；[Day 19](day19-pr387-code-review-prep.md) 保存 code rationale/test matrix；[Day 30](day30-pr387-warm-pool-dataflow-review.md) 保存 warm-pool adoption、conflict rebase、test-only E2E、upstream push 和 runner 复核。PR head `e32a463` 已通过 official 13/13；单行 runner follow-up `df8eb9b` 将 E2E 从唯一残留的 Ubuntu 22.04 对齐到 24.04，fork 9/9 且两个 E2E target PASS | #433 已有人推进，不修改认证/RBAC/WorkloadManager 产品逻辑。确认后把 `df8eb9b` fast-forward 到 #387；之后等待 maintainer `lgtm`/`approved`，不自动评论或继续堆提交 |
