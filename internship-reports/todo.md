...
| P0 | 修复 Release Image CI 的 Helm chart `latest` version 失败 | TODO | 当前无 assignee；关联 release workflow | 中 | 低 | 0.5 天 | [Day 38](day38-release-image-ci-helm-chart-version-failure-analysis.md) 已确认 `Build and Push Release Images` 在 main push 上长期失败：workflow 将 `TAG=latest` 同时用于 Docker image tag 和 Helm chart version，导致 `helm package --version latest` 报 `chart.metadata.version "latest" is invalid`；最近 50 个 main push run 均为 failure，抽样失败步骤为 `Package Helm chart` | 先向用户确认是否开独立 upstream PR；推荐小 PR：拆分 `IMAGE_TAG` / `CHART_VERSION`，main 只推 `latest` images，Helm chart login/package/push 只在 tag release 执行 |
