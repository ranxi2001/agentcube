...
| P0 | 适配 current stable `kubernetes-sigs/agent-sandbox` | REVIEW | #386；PR [#387](https://github.com/volcano-sh/agentcube/pull/387)；Go 前置 PR [#391](https://github.com/volcano-sh/agentcube/pull/391) 已合并 | 高 | 低 | 1-2 天 | [Day 16](day16-agent-sandbox-latest-adaptation.md) 保存 v0.4.6 适配与 runtime 验证；[Day 17](day17-pr387-copilot-review-and-ci-triage.md) 保存 review/CI triage；[Day 19](day19-pr387-code-review-prep.md) 保存 code rationale/test matrix；[Day 30](day30-pr387-warm-pool-dataflow-review.md) 保存 warm-pool adoption、conflict rebase、test-only E2E 收敛和 upstream push。用户确认后 PR head 已 fast-forward 到 `e32a463`；仅新增 workflow/E2E 三文件，保留 mTLS、增加 focused CodeInterpreter、校验 v0.4.6 和 UID adoption/cleanup/refill；fork 9/9、official 13/13 checks success，目标 WarmPool 测试实际 PASS | #433 已有人推进，不修改认证/RBAC/WorkloadManager 产品逻辑。当前等待 maintainer `lgtm`/`approved`，不自动评论、请求 review 或继续堆提交，不把 v0.5.0 迁移塞入 #387 |
