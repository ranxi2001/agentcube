...
| P0 | 建立 maintainer review / writing 历史学习闭环 | DONE | 不适用 | 中高 | 低 | 0.5 天 | [Day 46](day46-rainbowmango-maintainer-review-method-study.md) 已从 RainbowMango 的 15 个有效 review 样本提炼 problem-first、scope、ownership、routing、status transition 和 second-round 方法；追加 `@zhzhuang-zju` 29 个 issue/PR 分层样本，提炼 evidence-to-work、umbrella ledger、problem-to-behavior、migration/benchmark contract，并新增 writing history 抽取脚本和 4 个单测 | 下一次真实 review / issue / PR 前分别使用 problem card、kind/scope/destination matrix、issue evidence card 和 PR behavior card；只把跨样本或真实纠正升级为 skill |
...
| 完成 maintainer issue / PR 写作方法追加研究 | [Day 46](day46-rainbowmango-maintainer-review-method-study.md) 已对 `@zhzhuang-zju` 的 17 个 issue 与 12 个 PR 做跨类型、跨仓库采样，记录 issue/PR 上下文分工、因果证据链、umbrella 台账、兼容/benchmark 合同与不可复制反例，并升级 issue/PR writing skills |
