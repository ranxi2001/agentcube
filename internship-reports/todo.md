...
| P0 | 修复 Release Image CI 的 Helm chart `latest` version 失败 | DOING | 当前无 assignee；关联 release workflow | 中 | 低 | 0.5 天 | [Day 38](day38-release-image-ci-helm-chart-version-failure-analysis.md) 已确认 `Build and Push Release Images` 在 main push 上长期失败：workflow 将 `TAG=latest` 同时用于 Docker image tag 和 Helm chart version，导致 `helm package --version latest` 报 `chart.metadata.version "latest" is invalid`；已在 `/tmp/agentcube-release-chart-fix` 创建分支 `ci/fix-release-chart-version`，commit `dd7cb96 ci: avoid packaging Helm chart with latest version` 只改 `.github/workflows/build-push-release.yml`；本地 `git diff --check`、`actionlint` 目标检查、`helm lint`、`v1.2.3` / `v1.2.3-alpha` package 验证通过；fork 分支已推送到 `origin/ci/fix-release-chart-version` | 等用户确认是否开独立 upstream PR；PR 口径：main 只推 `latest` images，Helm chart login/package/push 只在 tag release 执行，tag `v1.2.3` 对应 chart version `1.2.3` |
