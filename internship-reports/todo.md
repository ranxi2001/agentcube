...
| P0 | 调研并准备 Release Image 多架构构建性能优化 | DOING | 待定；建议先开数据型 issue，再决定 PR | 中 | 中 | 0.5-1 天 | [Day 39](day39-karmada-image-build-and-agentcube-buildx-performance-optimization.md) 已对比 Karmada 和 AgentCube image build：Karmada 先在宿主 runner 上 `GOARCH=amd64/arm64` 交叉编译，再用 runtime-only buildx Dockerfile 组装镜像；AgentCube 当前在 buildx `linux/arm64` builder stage 中跑 Go compiler。旧 upstream job 中 `workloadmanager` arm64 `go build` 用 `1415.0s`；fork run `28633043101` 中 `Build and push images` 用 `1487.1s`，其中 `workloadmanager` arm64 `go build` 用 `1291.4s`。Day39 已按 issue-first 思路补充验证矩阵、#264 / #416 关系和英文 enhancement issue 草稿 | 新建本地验证分支 `ci/native-build-platform-images`，只改 Dockerfiles 跑本地 buildx smoke；如果 Scheme A 数据明确，再让用户确认 exact issue title/body 后开 upstream issue |
