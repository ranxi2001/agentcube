...
| P0 | 删除未使用的 `agentd` 组件 | REVIEW | upstream PR [#403](https://github.com/volcano-sh/agentcube/pull/403) | 中 | 低 | 0.5 天 | [Day 29](day29-agentd-component-role-analysis.md) 已记录分析、mentor 确认、fork CI 和 upstream PR；PR 分支基于 `upstream/main bed6bd4`，commit `31b840a cleanup: remove unused agentd component` 已创建 upstream PR #403；删除 `cmd/agentd` / `pkg/agentd`，移除 `build-agentd`，同步 docs/e2e wording，修正 `Picod (Agent Daemon)` 语义残留；fork PR #8 全绿；验证通过无 `agentd` / `Agent Daemon` 残留扫描、非 e2e Go tests、`make lint`、`make build-all`、`make gen-check`、e2e package 静态检查、docs build、`git diff --check` | 跟踪 #403 CI、AI review、OWNER/reviewer/tide；任何 push/comment 前先确认 scope 和 exact 内容 |
