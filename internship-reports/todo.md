...
| P0 | 前沿适配测试 `agent-sandbox v0.5.x` / `v1beta1` | REVIEW | #386；后续独立 follow-up，当前不绑定 upstream PR | 高 | 低 | 1-2 天 | [Day 18](day18-agent-sandbox-v05-forward-adaptation.md) 已完成 `v0.5.0rc1` 最小编译适配、clean runtime 验证和 fork CI 验证：fork branch `test/agent-sandbox-v05-forward` 当前 head `c0122da`，fork PR [#5](https://github.com/ranxi2001/agentcube/pull/5) 全绿；第一次 CI e2e 失败溯源为 e2e setup 仍安装旧 agent-sandbox manifest，集群无 v1beta1 CRD，已在 `c0122da` 对齐 `AGENT_SANDBOX_VERSION=v0.5.0rc1`；隔离 k3d + rc1 manifest 下 direct/warm-pool e2e、留存式 v1beta1 字段检查、delete cleanup、SDK、LangChain、MCP HTTP/stdio、math-agent LLM e2e 均通过；现有 v1alpha1 CRD 集群 server dry-run 会因 storedVersions 阻塞原地 apply | 等正式 `v0.5.0` release 或 maintainer 明确要求 rc support 后准备独立 upstream PR；PR 材料必须说明 clean-install 已验证、v1alpha1 -> v1beta1 原地升级未覆盖 |
| P0 | 项目二次梳理学习 | DONE | 不适用 | 中 | 低 | 0.5 天 | [Day 20](day20-project-second-pass-architecture-and-dependencies.md) 已完成：按 Router、WorkloadManager、Store、PicoD、agent-sandbox、codegen、auth、测试分层重新梳理当前 main 的真实实现；并补充 `agent-sandbox v0.2.1` / `v0.3.10` 分段适配实验，形成从 dependency-only 到 warm-pool adoption / codegen drift 的演进证据；分段实验已创建 fork-only 归档 PR [#6](https://github.com/ranxi2001/agentcube/pull/6) / [#7](https://github.com/ranxi2001/agentcube/pull/7)，fork CI 均全绿 | 后续改代码前按 Day20 的阅读顺序和测试矩阵定位影响面；#387 review 可引用 Day20 的 0.2/0.3/0.4 对比 |
...
| P2 | 深读 `agentd`、`picod`、`router` 链路 | DONE | 不适用 | 中 | 低 | 1 天 | [Day 20](day20-project-second-pass-architecture-and-dependencies.md) 已补 session 到 Router reverse proxy、PicoD JWT/execute、AgentD delete-on-idle 的代码级笔记 | 后续如改具体模块，再按 Day20 的模块 checklist 做专项测试 |
| P2 | 梳理 CRD / DeepCopy / client-go 生成链路 | DONE | 不适用 | 中 | 低 | 0.5 天 | [Day 19](day19-pr387-code-review-prep.md) 和 [Day 20](day20-project-second-pass-architecture-and-dependencies.md) 已解释 `make gen-all`、`make gen-check`、CRD YAML、DeepCopy、client-go/informer/lister/fake client 的作用 | 后续 API 字段改动必须跑 `make gen-check` 并解释 generated diff |
