...
| P0 | 删除未使用的 `agentd` 组件 | REVIEW | 新 PR 分支 `cleanup/remove-unused-agentd` | 中 | 低 | 0.5 天 | [Day 29](day29-agentd-component-role-analysis.md) 已记录分析、mentor 确认和 cleanup PR 准备；PR 分支基于 `upstream/main bed6bd4`，commit `30dd584 cleanup: remove unused agentd component` 已推送到 fork；删除 `cmd/agentd` / `pkg/agentd`，移除 `build-agentd`，同步 docs/e2e wording；验证通过非 e2e Go tests、`make lint`、`make build-all`、`make gen-check`、e2e package 静态检查、docs build、`git diff --check` | 等用户确认 upstream PR title/body/target 后再创建 PR；当前不发 upstream |
