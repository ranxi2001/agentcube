...
| P0 | 适配 current stable `kubernetes-sigs/agent-sandbox` | REVIEW | #386；PR [#387](https://github.com/volcano-sh/agentcube/pull/387)；Go 前置 PR [#391](https://github.com/volcano-sh/agentcube/pull/391) 已合并 | 高 | 低 | 1-2 天 | [Day 16](day16-agent-sandbox-latest-adaptation.md) 保存 v0.4.6 适配与 runtime 验证；[Day 17](day17-pr387-copilot-review-and-ci-triage.md) 保存 review/CI triage；[Day 19](day19-pr387-code-review-prep.md) 保存 code rationale/test matrix；[Day 30](day30-pr387-warm-pool-dataflow-review.md) 保存 warm-pool adoption、conflict rebase、test-only E2E、runner 复核和 reviewer note。PR head `df8eb9b` 已对齐 Ubuntu 24.04，official 12/12 checks success；[Mermaid comment](https://github.com/volcano-sh/agentcube/pull/387#issuecomment-4966669956) 解释双 job 和 UID/cleanup 数据流 | #433 已有人推进，不修改认证/RBAC/WorkloadManager 产品逻辑。当前只等待 maintainer `lgtm`/`approved`，不自动评论、请求 review 或继续堆提交 |
