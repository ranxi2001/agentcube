...
| P0 | 分析 `cmd/agentd` 当前组件作用 | DONE | 不适用 | 中 | 低 | 0.5 天 | [Day 29](day29-agentd-component-role-analysis.md) 已基于 `upstream/main bed6bd4` 分析：当前 `agentd` 是 watch `agent-sandbox` `Sandbox` CR 的 annotation-driven idle cleanup controller，不是 PicoD，也不是 sandbox 内 code execution daemon；关键风险是它读 Kubernetes `last-activity-time` 注解，而当前 Router / WorkloadManager GC 主链路使用 Store last-activity index；追加确认官方 release workflow 只 push workloadmanager/router/picod，Helm chart 只部署 router/workloadmanager，e2e fixture 使用 picod/python，当前本机集群也无 agentd Pod/container | 后续如继续，追历史 PR / issue 判断它是否是早期 idle GC 遗留组件；更适合做 docs 澄清 PR |
