...
| P0 | 适配 current stable `kubernetes-sigs/agent-sandbox` | REVIEW | #386；PR [#387](https://github.com/volcano-sh/agentcube/pull/387)；Go 前置 PR [#391](https://github.com/volcano-sh/agentcube/pull/391) 已合并 | 高 | 低 | 1-2 天 | [Day 16](day16-agent-sandbox-latest-adaptation.md) 保存 v0.4.6 适配与 runtime 验证；[Day 17](day17-pr387-copilot-review-and-ci-triage.md) 保存 review/CI triage；[Day 19](day19-pr387-code-review-prep.md) 保存 code rationale/test matrix；[Day 30](day30-pr387-warm-pool-dataflow-review.md) 保存 warm-pool adoption、conflict rebase 和 test-only E2E 收敛。#387 current head `401a00e`；fork-only candidate `e826041` 仅改 workflow/E2E 三个文件，保留 mTLS job并新增 focused CodeInterpreter job，对齐 v0.4.6，按 UID 验证预热 Pod adoption、session cleanup 和 refill；fork 9/9 workflows success，目标 WarmPool 测试实际 PASS | #433 已有人推进，不修改认证/RBAC/WorkloadManager 产品逻辑。下一步先向用户展示 exact test-only diff；取得确认后才更新 #387 branch，不自动评论或请求 review，不把 v0.5.0 迁移塞入 #387 |
