...
| P0 | 调研并准备 Release Image 多架构构建性能优化 | TODO | 待定；可作为 #416 后续独立 PR | 中 | 中 | 0.5-1 天 | [Day 39](day39-karmada-image-build-and-agentcube-buildx-performance-optimization.md) 已对比 Karmada 和 AgentCube image build：Karmada 先在宿主 runner 上 `GOARCH=amd64/arm64` 交叉编译，再用 runtime-only buildx Dockerfile 组装镜像；AgentCube 当前在 buildx `linux/arm64` builder stage 中跑 Go compiler，旧 job 中 `workloadmanager` arm64 `go build` 用 `1415.0s`。Day39 推荐第一版最小 PR 只改 3 个 Dockerfile 的 builder `FROM --platform=$BUILDPLATFORM ...`，避免 QEMU 执行 Go compiler | 等 #416 方向稳定后新建 clean branch `ci/native-build-platform-images`，只改 Dockerfiles，跑本地 buildx smoke 和 fork validation，再准备 upstream PR 文稿 |
