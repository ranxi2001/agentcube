...
| P0 | 修复 Release Image CI 的 Helm chart `latest` version 失败 | REVIEW | Upstream PR [#416](https://github.com/volcano-sh/agentcube/pull/416) | 中 | 低 | 0.5 天 | [Day 38](day38-release-image-ci-helm-chart-version-failure-analysis.md) 已确认 `Build and Push Release Images` 在 main push 上长期失败：workflow 将 `TAG=latest` 同时用于 Docker image tag 和 Helm chart version，导致 `helm package --version latest` 报 `chart.metadata.version "latest" is invalid`；修复分支 `/tmp/agentcube-release-chart-fix` / `ci/fix-release-chart-version` 已更新为 commit `bfbcab9 ci: publish release artifacts only for tags`，只改 `.github/workflows/build-push-release.yml`：release artifacts 仅由 `v*.*.*` / `v*.*.*-*` tag 触发，main merge 不再 build/push release images，tag release 下 chart version 从 `v1.2.3` 规范化为 `1.2.3`；本地 `git diff --check`、`actionlint` 目标检查、`helm lint`、`v1.2.3` / `v1.2.3-alpha` package 验证通过；fork `main` push 验证创建 9 个验证 workflow、未创建 `Build and Push Release Images`、9/9 成功；PR #416 已创建，open、非 draft、mergeable、DCO success | 等待 PR #416 CI、maintainer review、`/lgtm`、`/approve` 和 tide；不自动 push 或评论，任何后续 upstream-facing 更新先确认 exact 内容 |
