...
| P0 | 设计 docs/proposals 提案管理入口 | DONE | 不适用；fork branch `docs/proposals-management` | 中 | 低 | 0.5 天 | [Day 37](day37-docs-proposal-directory-management.md) 已调研 AgentCube 历史提案存放方式和 Karmada `docs/proposals` 结构；基于 `upstream/main f9c37d5` 创建分支 `docs/proposals-management`，commit `21f9159` 新增 `docs/proposals/README.md`、`docs/proposals/proposal-template.md` 并更新 `CONTRIBUTING.md`；按用户反馈，README 不维护新增 proposal 全量索引，只索引遗留 `docs/design` 旧提案 | 如要创建 upstream PR，先让用户确认 exact title/body/target；当前不移动历史 `docs/design` 文件，避免破坏链接 |
