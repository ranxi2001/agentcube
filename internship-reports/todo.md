...
| P0 | 设计 docs/proposals 提案管理入口 | DONE | PR #415 | 中 | 低 | 0.5 天 | [Day 37](day37-docs-proposal-directory-management.md) 已调研 AgentCube 历史提案存放方式和 Karmada `docs/proposals` 结构；基于 `upstream/main f9c37d5` 创建分支 `docs/proposals-management`，commit `2cc5732` 新增 `docs/proposals/README.md`、`docs/proposals/proposal-template.md` 并更新 `CONTRIBUTING.md`；按用户反馈，README 不维护新增 proposal 全量索引，只索引遗留 `docs/design` 旧提案；新 proposal 使用 `docs/proposals/<proposal-name>/README.md` 统一目录式布局，optional 字段已显式标注 | 已创建 upstream PR [#415](https://github.com/volcano-sh/agentcube/pull/415)；后续等待 CI、maintainer review、`/lgtm`、`/approve`，不要自动 push/comment |
