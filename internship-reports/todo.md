...
| P0 | 修复 Release Image CI 的 Helm chart `latest` version 失败 | REVIEW | Upstream PR [#416](https://github.com/volcano-sh/agentcube/pull/416) | 中 | 中 | 0.5 天 | [Day 38](day38-release-image-ci-helm-chart-version-failure-analysis.md) 已确认 `Build and Push Release Images` 在 main push 上长期失败：workflow 将 `TAG=latest` 同时用于 Docker image tag 和 Helm chart version，导致 `helm package --version latest` 报 `chart.metadata.version "latest" is invalid`。收到 `zhzhuang-zju` review 后已废弃 tag-only 旧方案，当前最终方案为保留 `main` trigger 和 latest image 发布，同时拆分 Helm metadata：`main` push 使用 `TAG=latest`、`CHART_VERSION=0.0.0`、`APP_VERSION=latest`，release tag 保留现有 `vX.Y.Z` 命名。本地验证通过 `git diff upstream/main --check`、`actionlint`、`helm lint`、Helm package simulations；fork run [`28633043101`](https://github.com/ranxi2001/agentcube/actions/runs/28633043101) 成功。用户确认后已 force-with-lease 更新 upstream PR #416 到 commit `427e618`，title/body 已改成 latest image + chart `0.0.0` 口径，并回复 reviewer comment。Copilot 提醒固定 `0.0.0` 重复发布可能不唯一；用户确认后已直接回复，说明固定 `0.0.0` 是 latest/development chart 稳定入口，若 maintainer 要 immutable snapshot 再切 `0.0.0-main.<run_number>`。#416 最新所有实际 checks 均 success，PR `mergeable=MERGEABLE` | 等待 maintainer review、`/lgtm`、`/approve` 和 tide；之后处理 fork `main` 恢复问题 |
| P0 | 调研并准备 Release Image 多架构构建性能优化 | REVIEW | Issue [#419](https://github.com/volcano-sh/agentcube/issues/419); PR [#420](https://github.com/volcano-sh/agentcube/pull/420) | 中 | 中 | 0.5-1 天 | [Day 39](day39-karmada-image-build-and-agentcube-buildx-performance-optimization.md) 已对比 Karmada 和 AgentCube image build：Karmada 先在宿主 runner 上 `GOARCH=amd64/arm64` 交叉编译，再用 runtime-only buildx Dockerfile 组装镜像；AgentCube 当前在 buildx `linux/arm64` builder stage 中跑 Go compiler。旧 upstream job 中 `workloadmanager` arm64 `go build` 用 `1415.0s`；fork run `28633043101` 中 `Build and push images` 用 `1487.1s`，其中 `workloadmanager` arm64 `go build` 用 `1291.4s`。fork-only GitHub runner benchmark：baseline run `28636642514` 三镜像 buildx 合计 `1588s`、workloadmanager arm64 Go build `1404.5s`；Scheme A run `28636642076` 三镜像 buildx 合计 `308s`、workloadmanager arm64 Go build `169.4s`；总 buildx 命令耗时提升 `5.16x`，workloadmanager arm64 Go build 提升 `8.29x`。用户确认后已创建 #419 并追加 [scope comment](https://github.com/volcano-sh/agentcube/issues/419#issuecomment-4873610799)。已创建 upstream PR #420，commit `0f58bdb`，只改 3 个 Dockerfile 的 builder `FROM --platform=$BUILDPLATFORM`，不改 release workflow / Helm chart / matrix-cache / PicoD runtime layer；创建前通过 `git diff --check`、`make build-all`、三个 Dockerfile builder-stage multi-arch buildx smoke、fork push validation 9/9 success | 等待 #420 upstream checks、review、`/lgtm`、`/approve` 和 tide；不自动 push/comment；#419 继续跟踪 Karmada-style prebuild、PicoD runtime layer、matrix/cache follow-up |
...
| P0 | 分析社区 issue / PR 动态 | DOING | 按 issue 逐项记录 | 中 | 中 | 0.5-1 天 | [Day 9](day9-open-source-community-and-fork-sync.md)、[Week 2](week2-summary.md) 已完成两轮统计；[Day 23](day23-agentcube-future-architecture-and-design.md) 已把 2026-06-23 最新 PR / issue 信号整理成未来架构路线；[Day 26](day26-week3-community-latest-and-two-layer-architecture-bug-surface.md) 已把 2026-06-24 最新 issue 和 Day22-25 汇总成双层架构问题面；`design.md` 已基于 Day28 Substrate 复核升级为 Session lifecycle / Router activation / Store CAS / RuntimeProvider / AgentSlot 主线；[Day 40](day40-agentcube-latest-community-pr-issue-trends.md) 已更新 2026-07-06 社区快照，确认近期主线是 CI/release/workflow hardening、docs onboarding、PicoD observability 和若干 runtime/SDK correctness PR | 重点跟进 #420 review 状态；可选复测并 review #400；#405 如参与应聚焦文档与实现一致性；避免重复实现 #406/#397/#394/#395/#388/#303 等已认领任务 |
