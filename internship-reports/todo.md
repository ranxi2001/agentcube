...
| P0 | 适配 current stable `kubernetes-sigs/agent-sandbox` | REVIEW | #386；PR [#387](https://github.com/volcano-sh/agentcube/pull/387)；Go 前置 PR [#391](https://github.com/volcano-sh/agentcube/pull/391) 已合并 | 高 | 低 | 1-2 天 | [Day 16](day16-agent-sandbox-latest-adaptation.md) 保存 v0.4.6 适配与 runtime 验证；[Day 17](day17-pr387-copilot-review-and-ci-triage.md) 保存 review/CI triage；[Day 19](day19-pr387-code-review-prep.md) 保存 code rationale/test matrix；[Day 30](day30-pr387-warm-pool-dataflow-review.md) 保存 warm-pool adoption 数据流、k3d/SDK/MCP/math-agent 证据和 2026-07-14 conflict rebase。#387 body 已压缩为 266 visible words / 16 lines；5 个 DCO commits 已 rebase 到 `upstream/main@3de1272`，唯一 `go.sum` 冲突按目标 `go.mod` + `go mod tidy` 解决，用户确认后 head `c2633c5 -> 401a00e`；GitHub 回读 `mergeable=true` / `rebaseable=true`，11/11 checks 全绿，Tide 不再报 conflict | 等待 maintainer `lgtm` / `approve`；不自动评论或请求 review，不把正式 `v0.5.0` 迁移塞进 #387；v0.5 follow-up 继续按 Day41 独立处理 |
