...
| P0 | 前沿适配测试 `agent-sandbox v0.5.x` / `v1beta1` | DOING | #386；后续独立 follow-up，当前不绑定 upstream PR | 高 | 低 | 1-2 天 | [Day 18](day18-agent-sandbox-v05-forward-adaptation.md) 已创建，记录 v0.5.x 前沿适配边界、最小迁移假设、编译测试计划、runtime/e2e 验证矩阵和 Sleep/Resume 边界 | 先做 local/fork-only validation：审计 release/module/CRD 差异，创建前沿测试分支，完成最小编译适配；正式 upstream PR 等 `v0.5.0` release 或 maintainer 明确要求 |
