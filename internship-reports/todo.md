...
| P0 | 适配最新 `kubernetes-sigs/agent-sandbox` | DOING | #386；PR [#387](https://github.com/volcano-sh/agentcube/pull/387)；Go 前置 PR [#391](https://github.com/volcano-sh/agentcube/pull/391) 已合并 | 高 | 低 | 1-2 天 | [Day 16](day16-agent-sandbox-latest-adaptation.md) 已完成 `agent-sandbox v0.4.6` 适配、codegen 修复、k3s / SDK / MCP / math-agent 验证；[Day 17](day17-pr387-copilot-review-and-ci-triage.md) 已完成 Copilot/Gemini 评论分组、CI triage、#391 rebase validation、fork CI、upstream #387 update；当前 #387 checks 全绿；`zhzhuang-zju` 已评论要求确认 PR kind 并解释为什么不直接适配 `v0.5.0rc1` | 先让用户确认 upstream-facing 动作：建议 `/remove-kind bug` + `/kind feature`，更新 PR body 的 target-version note，并回复 rc1 源码分析；`v0.5.x` / `v1beta1` 迁移建议作为单独 follow-up，不混入 #387 |
