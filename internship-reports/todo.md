...
| P0 | 调研并准备 Release Image 多架构构建性能优化 | DOING | 待定；建议先开数据型 issue，再决定 PR | 中 | 中 | 0.5-1 天 | [Day 39](day39-karmada-image-build-and-agentcube-buildx-performance-optimization.md) 已对比 Karmada 和 AgentCube image build：Karmada 先在宿主 runner 上 `GOARCH=amd64/arm64` 交叉编译，再用 runtime-only buildx Dockerfile 组装镜像；AgentCube 当前在 buildx `linux/arm64` builder stage 中跑 Go compiler。旧 upstream job 中 `workloadmanager` arm64 `go build` 用 `1415.0s`；fork run `28633043101` 中 `Build and push images` 用 `1487.1s`，其中 `workloadmanager` arm64 `go build` 用 `1291.4s`。本地 clean worktree `ci/native-build-platform-images` 已完成 Scheme A smoke：只改 3 个 Dockerfile 的 builder `FROM --platform=$BUILDPLATFORM`，workloadmanager / router / picod 三个 multi-arch buildx 均成功，arm64 目标 Go build 均显示为 `linux/amd64->arm64 builder`；原始日志已归档到 `benchmarks/day39-release-image-build/`。PicoD 仍暴露独立 runtime 瓶颈：arm64 Ubuntu `apt-get install python3` 在 QEMU 下耗时 `1111.1s`。真实 PR 分支已按标准整理并推到 fork：`ranxi2001/agentcube:ci/native-build-platform-images`，commit `a20ab07 ci: build multi-arch images with native Go builder`，DCO 已签，常规 push CI 9/9 success。已创建 fork-only GitHub runner benchmark 分支：baseline `ci/image-build-baseline-benchmark` commit `4e737fc` run `28636642514`；Scheme A `ci/image-build-scheme-a-benchmark` commit `60e5849` run `28636642076`；两者只用 `Image Build Benchmark` workflow 的日志和 artifact 做性能数据 | 等两个 benchmark workflow 完成后提取 workloadmanager / router / picod 分段耗时，对比 baseline / Scheme A；之后准备 upstream issue exact title/body 给用户确认，不创建 self-PR，不把临时 workflow 混进真实 PR 分支 |
