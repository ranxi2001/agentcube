...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 3A | Store / GC split：把 idle `Ready` 从 delete 改成 pause candidate；`Paused` 超过 `pauseTimeout` 后 delete；`maxSessionDuration` 对所有 active 状态优先 delete | TODO | 适合做设计和 fake-provider 单测；代码实现需等维护者分工 | 正式 upstream 代码建议等 #387 合并；本地 spike 不必等 | 中等风险：这是核心实现路径，若 FAUST-BENCHOU 接手，我们应提供 review/test matrix，不直接抢 PR | unit：`ready` idle -> pause、不 delete；`paused` 未过期 keep；`paused` 过期 delete；max TTL 覆盖 ready/paused/resuming/failed；unknown status 不 destructive delete；Store CAS / pause expiry index | 设计 + 单元测试 + review checklist；后续可转代码 | 先把 Day24 GC decision table 转成 test case skeleton；如果写代码，只在 fork/local spike 做，不发 upstream |
