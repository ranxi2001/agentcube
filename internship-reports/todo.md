...
| P0 | 适配 current stable `kubernetes-sigs/agent-sandbox` | REVIEW | #386；PR [#387](https://github.com/volcano-sh/agentcube/pull/387)；Go 前置 PR [#391](https://github.com/volcano-sh/agentcube/pull/391) 已合并 | 高 | 低 | 1-2 天 | [Day 16](day16-agent-sandbox-latest-adaptation.md) 已完成 `agent-sandbox v0.4.6` 适配、codegen 修复、k3s / SDK / MCP / math-agent 验证；[Day 17](day17-pr387-copilot-review-and-ci-triage.md) 已完成 Copilot/Gemini 评论分组、CI triage、#391 rebase validation、fork CI、upstream #387 update；当前 #387 checks 全绿；PR kind/body/label 已改为 stable `v0.4.6` compatibility feature 口径 | 继续推进 #387 review/merge；不把 rc1 / v1beta1 扩入 #387 |
| P0 | 前沿适配测试 `agent-sandbox v0.5.x` / `v1beta1` | DOING | #386；后续独立 follow-up，当前不绑定 upstream PR | 高 | 低 | 1-2 天 | [Day 18](day18-agent-sandbox-v05-forward-adaptation.md) 已创建；local-only branch `test/agent-sandbox-v05-forward` commit `ee1aecf` 已完成 `v0.5.0rc1` 最小编译适配；通过 `pkg/workloadmanager`、cmd/agentd、e2e 空编译、非 e2e Go 全量、race、lint、gen-check、build-all、coverage 命令 | 进入 runtime-driven validation：确认 rc1 manifests/CRDs，安装真实 agent-sandbox controller，跑 direct/warm-pool e2e、GC cleanup、SDK/MCP/math-agent；正式 upstream PR 等 `v0.5.0` release 或 maintainer 明确要求 |
