...
| P0 | 调研并准备 Release Image 多架构构建性能优化 | REVIEW | Upstream issue [#419](https://github.com/volcano-sh/agentcube/issues/419) | 中 | 中 | 0.5-1 天 | [Day 39](day39-karmada-image-build-and-agentcube-buildx-performance-optimization.md) 已对比 Karmada 和 AgentCube image build：Karmada 先在宿主 runner 上 `GOARCH=amd64/arm64` 交叉编译，再用 runtime-only buildx Dockerfile 组装镜像；AgentCube 当前在 buildx `linux/arm64` builder stage 中跑 Go compiler。旧 upstream job 中 `workloadmanager` arm64 `go build` 用 `1415.0s`；fork run `28633043101` 中 `Build and push images` 用 `1487.1s`，其中 `workloadmanager` arm64 `go build` 用 `1291.4s`。本地 clean worktree `ci/native-build-platform-images` 已完成 Scheme A smoke：只改 3 个 Dockerfile 的 builder `FROM --platform=$BUILDPLATFORM`，workloadmanager / router / picod 三个 multi-arch buildx 均成功，arm64 目标 Go build 均显示为 `linux/amd64->arm64 builder`；真实 PR 分支已按标准整理并推到 fork：`ranxi2001/agentcube:ci/native-build-platform-images`，commit `a20ab07`，DCO 已签，常规 push CI 9/9 success。fork-only GitHub runner benchmark 已完成：baseline run `28636642514` 三镜像 buildx 合计 `1588s`、workloadmanager arm64 Go build `1404.5s`；Scheme A run `28636642076` 三镜像 buildx 合计 `308s`、workloadmanager arm64 Go build `169.4s`；总 buildx 命令耗时提升 `5.16x`，workloadmanager arm64 Go build 提升 `8.29x`。PicoD 仍暴露独立 runtime 瓶颈：arm64 Ubuntu `apt-get install python3` 约 `119.1s`，应拆 follow-up；原始日志和摘要已归档到 `benchmarks/day39-release-image-build/github-benchmark/`；用户确认后已创建 issue #419，已通过 `/assign` 认领，并通过 `/kind enhancement` 添加 `kind/enhancement` label；已追加 [scope comment](https://github.com/volcano-sh/agentcube/issues/419#issuecomment-4873610799)，明确第一阶段 PR 只做 `$BUILDPLATFORM` builder stage 最小改动，其余优化拆 follow-up | 等待 maintainer 对 issue #419 的方向反馈；如果认可最小方案，再把 `ci/native-build-platform-images` 整理成最小 PR；不创建 self-PR，不把临时 workflow 混进真实 PR 分支 |
