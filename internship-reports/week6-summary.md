...
CI 绿色必须同时回答“装了哪个 runtime”和“目标测试是否真正运行”。#387 原 E2E 一边安装旧版 agent-sandbox，一边因默认 mTLS 跳过 CodeInterpreter；只有版本和执行路径同时对齐，绿色结果才支持兼容性结论。

timer 不会自动终止同一 goroutine 中的阻塞 I/O。deadline 必须进入 client-go GET 使用的 context，并在返回成功前再次检查；否则 2 分钟 timer 已到期后，迟到的 Ready 结果仍可能被当作成功。

flake 修复不能停在“重跑通过”。需要从 terminal assertion 反向连接 producer、缓存或状态传播、consumer 和恢复事件，再让补丁切断同一条 causal edge；否则增加 timeout 或 retry 只会隐藏问题。
