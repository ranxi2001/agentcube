# Week 6 总结：agent-sandbox 新版适配、Karmada 修复与 PR Review
...
| 目标 | 状态 |
| --- | --- |
| 完成 AgentCube 对 agent-sandbox v0.5.2 的适配验证 | 进行中 |
| 修复 Karmada 证书轮换与偶发 E2E 失败 | 进行中 |
| 参与 AgentCube/Karmada PR Review 与 Feature Proposal Review | 进行中 |

### 2. 本周进展

| 工作项 | 结果、价值或剩余风险 | 完成时间 | 状态 |
| --- | --- | --- | --- |
| AgentCube 适配 agent-sandbox 新版 | PR #387 于 7 月 16 日合并，CI 12/12 通过；agent-sandbox v0.5.2 已在全新集群完成沙箱创建、调用和删除验证，旧集群升级验证转入后续范围 | 7/17 | 已完成 |
| Karmada 证书轮换 #7697 | 已提交修复，覆盖证书轮换时可能丢失访问地址、误用其他集群凭据及更新中断后无法继续的问题；CI 17/17 通过，等待维护者审核 | 7/17 | 进行中 |
| Karmada 偶发 E2E 失败 | PR #7732 合并并关闭 Issue #7719；另找到一个偶发失败的原因：状态变化没有触发控制器重新处理任务，并创建 Issue #7776 和修复 PR #7777 | 7/17 | 进行中 |
| AgentCube/Karmada PR Review | 参与 6 个 PR 的 Review，发布 18 条 inline comments；#400 作者按意见修复监控数据项持续增加的问题，#431 作者处理 9 条设计问题，另通过 #439 提名 RainbowMango 为维护者 | 7/17 | 已完成 |
...
AgentCube 适配 agent-sandbox v0.5.2 时，应在同一个 PR 中完成旧集群升级验证，还是先支持全新安装，再单独处理旧集群升级？

### 5. 下周计划

| 任务 | 可验收结果 |
| --- | --- |
| agent-sandbox v0.5.2 适配 PR Review（AgentCube PR #442） | 代码稳定后，检查 API 兼容性、旧集群升级、Claim/Sandbox 关联、Sandbox lifecycle 和 E2E，给出 merge 建议和剩余风险 |
| WarmPoolAvailable 状态与 Event（AgentCube PR #385） | agent-sandbox v0.5.2 适配合入主线后，只迁移 WarmPoolAvailable 相关的 6 个文件并重新验证，避免重复带入适配代码 |
| 证书轮换与 Remedy 状态修复（Karmada PR #7697、#7777） | 处理维护者意见，区分代码问题与 CI 环境故障，确认两个 PR 是否具备合并条件 |
| SandboxPool/WorkloadRebalancer Feature Proposal Review（AgentCube PR #431、Karmada PR #7662） | 两个 Feature Proposal 更新后逐项检查已有设计问题，输出可开始实现的范围和仍需确认的设计边界 |
...
| 参与 Review 的 PR | 6 | AgentCube #400、#431；Karmada #7692、#7662、#7764、#7623 |
| 完成深入分析的问题 | 2 | AgentCube #438、Karmada #7776 |
| Feature Proposal Review 意见 | 10 条提出 / 9 条讨论已关闭 | AgentCube #431 的 9 条；Karmada #7662 的迁移顺序问题 |
| inline comments | 18 | AgentCube 11 条；Karmada 7 条，其中 17 条初始问题、1 条补充说明 |

> 注释：PR Review 按仓库及编号去重。#387 的自审 inline comments、自动化评论、提醒、仅表示同意的评论、个人仓库提交和报告提交不计入 Review 数量。
