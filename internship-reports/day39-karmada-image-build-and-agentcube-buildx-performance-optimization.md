...
#### #420 rebase 后的 fork main 真实 release 验证

2026-07-07 按用户要求把 #420 分支临时推到 fork `main` 验证真实 release workflow。第一次直接把旧 PR head `0f58bdb` 指到 fork `main` 后，run [28838057204](https://github.com/ranxi2001/agentcube/actions/runs/28838057204) 失败在 `Package Helm chart`：

- `Build and push images` 已成功完成。
- 失败原因是旧 #420 head 没包含 #416 已合入的 chart version 修复，workflow 仍把 `TAG=latest` 写进 chart version。
- Helm 报错：`chart.metadata.version "latest" is invalid`。

随后将 #420 rebase 到最新 `upstream/main 7242754`，生成新 head：

```text
d996bbb ci: build multi-arch images with native Go builder
```

rebase 后 diff 仍然只包含三个 Dockerfile 的 builder stage 平台修改：
...
1. `git diff --check refs/remotes/upstream/main...HEAD`
2. force-with-lease 更新 `ranxi2001:ci/native-build-platform-images` 到 `d996bbb`
3. force-with-lease 更新 fork `main` 到 `d996bbb`
4. 取消旧 merge-commit `59552e6` 上仍在跑的重复 release / e2e workflow

fork `main` 真实 release workflow run [28838465210](https://github.com/ranxi2001/agentcube/actions/runs/28838465210) 成功：

| Item | Result |
| --- | --- |
| Head | `d996bbb9532c295f59cb1fc6856a78dd3f56b1c7` |
| Job | `build-and-push` |
| Job wall time | `03:03:39Z` -> `03:09:30Z`, about `5m51s` |
| `Build and push images` step | `03:03:59Z` -> `03:09:21Z`, about `5m22s` |
| workloadmanager image | about `184s`; `linux/amd64->arm64 go build ./cmd/workload-manager` `162.1s` |
| router image | about `11.7s`; `linux/amd64->arm64 go build ./cmd/router` `3.3s` |
| picod image | about `126.7s`; `linux/amd64->arm64 go build ./cmd/picod` `0.8s`; arm64 `apt-get install python3` `116.2s` |
| Helm chart | `CHART_VERSION=0.0.0`, packaged `agentcube-0.0.0.tgz`, pushed `ghcr.io/ranxi2001/charts/agentcube:0.0.0` |

同一 head 的 fork `main` push checks 全部成功，包括 `Build and Push Release Images`、`Agentcube CI Workflow`、`Agentcube E2E Tests`、`Lint`、`Codegen Check`、`Copyright Check`、`Codespell`、`Python Lint`、`Python SDK Tests` 和 `Test Coverage`。

upstream PR #420 也已更新到 `d996bbb`，changed files 仍为 3，PR mergeable，push 后 upstream PR checks 全绿；剩余门禁仍是 reviewer `lgtm` / `approved`。

用户确认后已在 PR #420 发布验证与原理说明 comment：<https://github.com/volcano-sh/agentcube/pull/420#issuecomment-4900010463>。评论重点解释 Docker BuildKit 的 `BUILDPLATFORM` / `TARGETPLATFORM` 语义、为什么 `--platform linux/arm64` 不会把 amd64 CI runner 变成 arm64、Dockerfile `RUN` 在 target-platform builder stage 中为什么会通过 QEMU 执行，以及本 PR 如何只让 Go compiler stage 跑在 native build platform，同时继续用 `TARGETOS` / `TARGETARCH` 交叉编译目标平台二进制。
