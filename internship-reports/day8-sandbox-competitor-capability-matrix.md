...
| 核心原语 | `CodeInterpreter` CRD + `SandboxWarmPool` + Router + Workload Manager | 从预热父 Firecracker VM fork 子 microVM，支持 BRANCH | RustVMM/KVM/PVM microVM + Template + Snapshot + CubeCoW + CubeVS + CubeEgress | 单 Rust binary，按 workspace 管理执行环境 |
...
| 性能优化方式 | Warm pool 预热 Pod / sandbox，减少 session 创建等待 | 父 VM 预热后 CoW fork，导入依赖和内存状态可继承 | 资源池预配置、CubeCoW reflink/soft-dirty snapshot、TAP pool、eBPF 数据面 | 轻量进程启动，低 runtime 管理开销 |
...
| API / SDK | Python SDK、Router HTTP、LangChain/Dify 等集成 | REST、Python SDK、TypeScript SDK、MCP，Python SDK 走 E2B 风格 | E2B 兼容 REST；官方 `cubesandbox` Python/Go SDK；template/snapshot/AgentHub API；Web UI | REST、MCP、Python/TS SDK，E2B lifecycle 部分兼容 |
| 快照 / 分支能力 | 当前报告未验证内存快照；主要是 session / warm pool | 支持 snapshot、diff chain、BRANCH/live BRANCH | 支持 snapshot、clone、rollback；snapshot ID 可作为 template 创建新 sandbox | 文件系统 workspace snapshot / restore / fork，不保存进程内存 |
| 当前本机状态 | 已跑通并完成延迟 benchmark | 未跑通，glibc 和 KVM 双重 blocker | CubeAPI 网关已编译启动且 `/health` OK；完整 sandbox 被无 `/dev/kvm`、glibc 2.28、缺 XFS `/data/cubelet`、PVM 需换内核挡住 | 已源码构建并跑通 |
| 适合优先评估的问题 | K8s 原生调度、session 生命周期、warm pool 命中率 | 单机 KVM 上大规模 agent fan-out 和状态继承 | E2B 替代、KVM/PVM 安全边界、多节点 sandbox、CubeCoW 状态管理、出网治理 | 轻量工具运行时、MCP/REST 工具密度、非强对抗代码 |
...
| Linux 云 VM，有 nested virt / `/dev/kvm` | 可作为 K8s 节点；可接 Kata/Kuasar 等 RuntimeClass | 可跑，官方 quick start 要求 x86_64 Linux + KVM + Ubuntu 22.04+ | 可跑标准部署；还需 root、Docker、glibc ≥ 2.31、XFS `/data/cubelet` | 可跑，安全边界仍是进程级 |
| 裸金属 / bare metal | 可作为 K8s 节点，适合跑强隔离 runtime | 官方 benchmark 环境就是 KVM Linux 主机 | 官方支持 bare-metal 部署，BMI5 官方 benchmark 数据最完整 | 可跑 |
...
| CentOS 8，本机现状 | 已实测 | 官方二进制因 glibc 2.28 不兼容；且无 `/dev/kvm` | CubeAPI-only 可跑；完整部署需要 glibc ≥ 2.31、KVM 或 PVM、XFS `/data/cubelet`，当前机器不适合 | 官方二进制 glibc 不兼容；源码构建成功，但 kernel 4.18 无 Landlock |
...
| CubeSandbox | 中到高：root、Docker、KVM/PVM、XFS `/data/cubelet`、模板构建、CubeMaster/Cubelet/Web UI 等组件 | 官方 quick start 主打无需源码构建；CubeAPI Rust 源码本机已编译通过 | 当前机器只跑通 CubeAPI `/health`；完整 sandbox 未跑 | 需要 KVM 或 PVM 内核、glibc ≥ 2.31、XFS；PVM 路线需要换内核和重启，不适合共享测试机 |
...
| 文件系统隔离 | 取决于 Pod spec、镜像、runtime 和 agent-sandbox 实现 | 子 microVM 独立文件系统/快照 | 模板和 microVM 文件系统；CubeCoW 支持 snapshot/clone/rollback | workspace 级；Linux Landlock 限制系统 allowlist 和 workspace |
| 网络隔离 | 取决于 Kubernetes NetworkPolicy / runtime / CNI | per-child netns；官方状态里 default-deny egress 仍是 production gap | CubeVS/eBPF L3/L4 策略 + CubeEgress L7 域名过滤、凭据注入、访问审计 | 项目自身不做网络隔离，交给外层 VM/容器/防火墙 |
...
| API 认证 | PicoD 模式注入 Router 公钥，Router 用短期 JWT 代理请求 | REST bearer auth、audit log；早期版本有安全修复，官方建议升级 | E2B API key 兼容；Dashboard 可保存 API key；CubeEgress 审计出网请求但控制面鉴权仍需单独部署验证 | README 强调 REST 可审计；未把 auth 当核心能力介绍 |
...
| CubeSandbox | 裸金属 BMI5：template 创建 1 并发 avg `47.8 ms` / p95 `57.4 ms`，10 并发 avg `88.7 ms` / p95 `116.9 ms`，20 并发 avg `98.1 ms` / p95 `175.8 ms`，50 并发 avg `276.1 ms` / p95 `508.4 ms`；1000 个存活沙箱均摊约 `25.7 MB` | 腾讯云 BMI5 裸金属，96 逻辑核，375 GiB，NVMe XFS，沙箱 2 vCPU / 2 GiB | 官方完整 benchmark 比 README 摘要更细；需要 KVM、资源池、XFS/CubeCoW |
| CubeSandbox | PVM 云服务器 SA9.4XLARGE32：1 并发 avg `66.7 ms` / P95 `78.2 ms`，10 并发 avg `170.9 ms` / P95 `216.7 ms`，20 并发 avg `364.6 ms` / P95 `521.4 ms`；单 VM 空载均摊约 `27-34 MB` | 腾讯云 SA9.4XLARGE32，16 逻辑核，32 GiB，PVM kernel `6.6.69-opencloudos9.cubesandbox.pvm.host`，沙箱 2 vCPU / 2 GiB | PVM 能覆盖普通云 VM，但需要安装 PVM host kernel 并重启；性能低于裸金属 |
...
| CubeSandbox | CubeAPI-only：编译并启动 Rust API gateway，访问 `GET /health` | 返回 `{"status":"ok","sandboxes":0}` | 未进入 sandbox benchmark | 只证明 API 网关层可运行；完整 sandbox 仍被无 `/dev/kvm`、glibc 2.28、缺 XFS `/data/cubelet`、PVM 需换内核挡住 |
...
- CubeSandbox PVM deployment：<https://github.com/TencentCloud/CubeSandbox/blob/master/docs/zh/guide/pvm-deploy.md>
- CubeSandbox bare-metal deployment：<https://github.com/TencentCloud/CubeSandbox/blob/master/docs/zh/guide/bare-metal-deploy.md>
- CubeSandbox performance benchmark：<https://github.com/TencentCloud/CubeSandbox/blob/master/docs/blog/posts/2026-06-01-cubesandbox-perf-benchmark.md>
- CubeSandbox PVM benchmark：<https://github.com/TencentCloud/CubeSandbox/blob/master/docs/blog/posts/2026-06-03-cubesandbox-perf-benchmark-pvm.md>
- CubeSandbox snapshot / rollback / clone：<https://github.com/TencentCloud/CubeSandbox/blob/master/docs/zh/guide/snapshot-rollback-clone.md>
- CubeSandbox WebUI：<https://github.com/TencentCloud/CubeSandbox/blob/master/docs/zh/guide/webui.md>
- CubeSandbox egress policy：<https://github.com/TencentCloud/CubeSandbox/blob/master/docs/zh/guide/egress-network-policy.md>
- CubeSandbox security proxy：<https://github.com/TencentCloud/CubeSandbox/blob/master/docs/zh/guide/security-proxy.md>
