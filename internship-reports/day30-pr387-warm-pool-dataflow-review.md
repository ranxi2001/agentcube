...
用户指出 Files Changed 页面的 inline comment 难读后，GraphQL 审计发现作者共发布 15 条
inline comment、约 29,034 个字符和 382 个非空行。其中 11 条是作者主动在文件第一行发起的
code-rationale 说明，没有 reviewer 回复，却占 27,583 个字符；它们把本应留在本地报告的
逐文件解释、调试时间线和完整测试矩阵搬进了 reviewer 的主阅读路径。

经用户确认，删除这 11 条零回复的独立说明，保留 4 条直接回应 Copilot/maintainer finding
的英文回复。删除后 GraphQL 回读为 4 条作者 comment：3 条 current、1 条因后续代码改动而
structurally outdated；4 条都仍属于原 reviewer thread，本轮没有 resolve、重发或新增评论。
PR head 仍为 `95fae1f`，12 项 checks 仍为 success，`MERGEABLE` 状态未变化。

> 分析：inline comment 的职责是让 reviewer 在具体代码位置完成一个判断，而不是承载完整
> file rationale。后者应留在本地 code-rationale matrix / 工程报告，PR body 只保留问题、行为、
> 关键风险和验证索引。GitHub 的 `isOutdated=false` 只说明锚点仍能映射到 diff，并不证明评论
> 里的旧行号、旧 SHA、旧 diff 统计仍然有效。

## 当前 head 的更新前后数据流图（2026-07-15）

[Mermaid 真源](day30-pr387-before-after-data-flow.mmd) | [白底 PNG](day30-pr387-before-after-data-flow.png)

![PR #387 warm-pool 更新前后数据流](day30-pr387-before-after-data-flow.png)

> 阅读方式：入口、创建 Claim、删除 Claim 是共同流程；从 `差异 1` 到 `差异 7` 横向比较
> 同编号节点，依次查看池化对象、adopt 对象、身份关系、就绪发现、Pod 读取、Store 身份和
> 补池对象如何变化。

> 注释：这里的“更新前”不再使用本报告前部的旧 E2E helper 观测假设，而是精确对应
> merge base `3de1272d` 中的 AgentCube 代码和 `agent-sandbox v0.1.1` controller；“更新后”
> 对应 PR 当前 head `95fae1f8` 与 `agent-sandbox v0.4.6`。图只比较 warm-pool session
> lifecycle，不展开 direct Sandbox、认证、依赖生成或 CI workflow。

更新前，`SandboxWarmPool` 直接维护预热 Pod。`SandboxClaim` controller 先 adopt 一个 Pod，
再创建与 Claim 同名的 `Sandbox`，因此 WorkloadManager 可以预注册
`Sandbox/<claim-name>` watcher，并从同名 Sandbox 的 Ready event 继续解析 Pod IP。此时
`Claim.name == Sandbox.name`，Store 的控制身份与 runtime 名称天然重合。

更新后，`SandboxWarmPool` 维护的是预热 `Sandbox -> Pod`。Claim controller adopt 的是已经
存在的 Sandbox，并保留其原名称和 UID；Claim 与 serving Sandbox 不再同名。WorkloadManager
因此改为读取 `SandboxClaim.status.sandbox.name`，再 live GET adopted Sandbox 并检查
`Ready=True`。最新 deadline 修复让同一个 2 分钟 wait context 覆盖 Claim GET 和 Sandbox GET，
避免阻塞 API call 绕过 timer 后提交迟到 success。

Store 仍必须保留 `Kind=SandboxClaim`、`Name=<claim name>` 和 session 自己的 TTL 时间，供
delete、GC 与 rollback 删除控制对象；`SandboxID` 与 `EntryPoints` 则来自 adopted Sandbox UID
和 Pod IP。删除 Claim 后，controller/ownerReference 清理 adopted runtime，warm pool 再新建
replacement Sandbox/Pod；原运行对象不会回到池中。这个“控制身份保持、运行身份切换”是
#387 的核心兼容设计。
...
## 2026-07-16：Approval、Tide Merge 与编号校准

用户同步“PR #386 已 merge”后回读官方对象，确认编号应校准为 **PR #387 已合并**；AgentCube
[#386](https://github.com/volcano-sh/agentcube/issues/386) 仍是 open 的 v0.2.0 umbrella issue，
`/pulls/386` 返回 404。不能因为 #387 来自 #386 的 adaptation proposal，就把 issue 编号写成
PR 编号或把 umbrella issue 记为已关闭。

最终 Prow 时间线：

| 时间（UTC） | 事件 | 结果 |
| --- | --- | --- |
| 2026-07-16 08:13:17 | `@RainbowMango` 移除 `tide/merge-method-squash` | 取消此前指定的 squash merge method |
| 2026-07-16 08:35:56 | [`@RainbowMango /approve`](https://github.com/volcano-sh/agentcube/pull/387#pullrequestreview-4711905661) | Prow approval gate 闭合 |
| 2026-07-16 08:36:19 | `approved` label 生效 | Tide 获得最后一个 required label |
| 2026-07-16 08:37:15 | `volcano-sh-bot` merge/close | PR #387 正式进入 `main` |

merge commit 是 `146b75fc4b98f214988b5d0c5059a55a2bc1f9da`，两个 parent 分别为旧
`main` `fa254b15` 和 exact PR head `95fae1f8`。因此它是 two-parent merge commit，不是
squash commit；PR feature branch 本身仍只有一个 DCO commit，且 `upstream/main` 直接包含
`95fae1f8`。从旧 main 到 merge commit 的 delta 仍是已审计的 18 个文件、`+1189/-312`，没有
合并时生成第三份代码 patch。

> 分析：merge-method label 是动态流程状态，不能只根据较早的 `/label tide/merge-method-squash`
> 评论推断最终历史形态。应回读 label event、merge commit parent 和
> main ancestry；这三者共同证明最终采用普通 merge。

本轮没有重新运行代码测试：合并对象就是 official 12/12 checks 与 DCO 已通过的 exact head，
这里只读核对 GitHub merge 状态、commit ancestry 和 diff。`upstream/main` 已前进到 `146b75f`；
个人 fork `origin/main` 当前落后 7 commits，本轮“观察”不授权自动 reset/push，因此保持不动。

### 合并后的工作边界

1. stable `agent-sandbox v0.4.6` compatibility 已成为 upstream baseline，#387 不再追加 commit、
   review request、thread resolve 或 auth/RBAC 修改。
2. #386 umbrella issue 仍 open，而且正文 checklist 尚未把 agent-sandbox adaptation 回填为已完成；
   这是 tracker freshness，不是 #387 代码问题，本轮不自动评论。
3. `agent-sandbox v0.5.0` adapter 可以在后续独立 scope 中基于新 main 重验，但不能把已完成的
   #387 重新扩张成 v0.5 migration。
4. Sleep/Resume 的技术前置虽然解除，ownership 仍需尊重 `FAUST-BENCHOU` 已表达的接手意愿和
   #386 后续 triage；#433 auth/RBAC 也继续由现有贡献者跟进。
