...
```text
2cc5732 docs: add proposal index and template
```
...
2. 新增 `docs/proposals/proposal-template.md`。
   - 包含 title、authors、reviewers、approvers、creation-date、last-updated、status、tracking issue optional。
   - 包含 Summary、Motivation、Goals、Non-Goals、Proposal、User Stories optional、Design Details、Risks and Mitigations、Test Plan、Alternatives、Implementation Plan optional。
...
模板中已显式用 `# Optional.` 标注，避免 reviewer 误以为每个 proposal 都必须有
tracking issue。
...
#### 7. 增加 `Implementation Plan (Optional)`

AgentCube 模板比 Karmada 多了可选 `Implementation Plan (Optional)`。
...
用户指出：虽然最初模板在 HTML comment 中写了 optional，但标题和字段本身没有显式
标注，reviewer 快速浏览时仍然不清楚哪些内容可选。

因此最终模板和 README 都已补齐 optional 标注：
...
当前远端 fork 分支已再次 force-with-lease 更新：`504bf34` 被 amend 为
`2cc5732`。本次只改 proposal template / README 的 optional 标注，验证命令为
`git diff --check`。
...
Optional fields and sections are explicitly marked in the template:
`tracking-issue: "" # Optional.`, `User Stories (Optional)`, and
`Implementation Plan (Optional)`.
```
