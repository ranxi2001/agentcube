...
```text
59ac668 docs: add proposal index and template
```
...
1. 新增 `docs/proposals/README.md`。
   ...
   - 规定新 proposal 统一采用 `docs/proposals/<area>/<proposal-name>/README.md` 目录式布局。
...
- New proposals use a directory-based layout such as
  `docs/proposals/sandbox/sandbox-pool-control-plane/README.md`, so images and
...
```text
docs/proposals/sandbox/sandbox-pool-control-plane/README.md
docs/proposals/runtime/e2b-compatible-sdk-facade/README.md
```
...
`59ac668`。本次只改 proposal layout 文档，验证命令为 `git diff --check`。
