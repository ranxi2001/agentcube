...
1. Day11 已补 CubeSandbox API 兼容、Python / Go SDK、template / snapshot API 和 CubeSandbox vs AgentCube 对比表。
2. Day8 竞品矩阵同步补齐 CubeSandbox 能力：PVM、CubeEgress、CubeCoW、Web UI、Go/Python SDK、官方 benchmark 和本机 CubeAPI-only 验证状态。
3. 当前测试机不适合直接安装完整 CubeSandbox；如需实测，准备 OpenCloudOS 9 / TencentOS 4、glibc ≥ 2.31、XFS `/data/cubelet`、KVM 或 PVM kernel 的独立机器。
4. 后续如果拿到合适机器，优先跑三类 smoke：创建 template、创建/删除 sandbox、snapshot -> clone -> rollback。
