...
| v0.4.0-rc | 2026-06-14 | CubeEgress、安全代理、日志转发、版本矩阵、模板兼容性、网络性能优化 |
...
| 维度 | CubeSandbox | AgentCube |
| --- | --- | --- |
| 核心优势 | 高性能 microVM sandbox、E2B 兼容、snapshot/clone/rollback、PVM | Kubernetes-native、CRD 抽象、workload/session 管理、云原生调度 |
| 当前成熟度 | 从 runtime 到 Web UI / SDK / 网络 / 安全代理较完整 | 社区早期，SnapStart / warm pool / E2B 兼容仍在设计和实现中 |
| 部署成本 | root、Docker、XFS、KVM 或 PVM kernel、独立 control plane | Kubernetes 集群，依赖 agent-sandbox / runtime provider |
| 云 VM 适配 | PVM 是亮点，但要换内核重启 | 可跑在普通 K8s，但强隔离依赖底层 runtime 能力 |
| API 生态 | 明确 E2B drop-in | 还需补 E2B / SDK / MCP 等生态兼容 |
| 多节点 | 自带 CubeMaster / Cubelet 集群 | 原生借 K8s 调度和控制面 |
| 安全边界 | MicroVM + eBPF + CubeEgress | 取决于 runtime；控制面可做策略编排 |
...
1. 继续围绕 CubeSandbox 补充 API 兼容细节：E2B SDK 兼容范围、CubeSandbox Python SDK 新增接口、模板和 snapshot API。
2. 把 CubeSandbox 的能力补进 Day8 竞品矩阵，尤其是 PVM、CubeEgress、CubeCoW、Web UI、Go/Python SDK 和官方 benchmark。
3. 单独建立 CubeSandbox vs AgentCube 的对比表：一边是底层 sandbox platform，一边是 Kubernetes-native 调度/管理层。
4. 当前测试机不适合直接安装 CubeSandbox；如需实测，准备 OpenCloudOS 9 / TencentOS 4、glibc ≥ 2.31、XFS `/data/cubelet`、KVM 或 PVM kernel 的独立机器。
5. 其他云厂商项目先保持项目名和入口占位，等待同学补充后再合并到总表。
