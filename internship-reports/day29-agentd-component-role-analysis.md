...
| commit | `314e138601849f397aa3fbebb9b9d43a8a6661a8 cleanup: remove unused agentd component` |
...
| upstream PR | [volcano-sh/agentcube#403](https://github.com/volcano-sh/agentcube/pull/403) |
...
> 注释：根据本地协作规则，创建 upstream PR、issue、review comment、maintainer mention 都需要用户明确确认目标仓库、title 和 body。本轮已在用户确认后创建 upstream PR。

### Upstream PR

用户确认 title/body/target 后，已创建 upstream PR：

- PR: <https://github.com/volcano-sh/agentcube/pull/403>
- Title: `cleanup: remove unused agentd component`
- Base: `volcano-sh/agentcube:main`
- Head: `ranxi2001:cleanup/remove-unused-agentd`
- Head SHA: `314e138601849f397aa3fbebb9b9d43a8a6661a8`
- Label: `kind/cleanup`
- 初始状态：`mergeable=true`，`mergeable_state=unstable`，表示等待 CI / review gate；不是 merge conflict。
- 初始 checks：DCO、Approve workflows、Python Lint、python-sdk-tests 已通过；codespell、Codegen Check、build、coverage、e2e-test、golangci-lint 正在跑。

Gemini review 反馈处理：

- 反馈位置：`docs/agentcube/docs/developer-guide/building-runtimes.md` 的 PicoD copy 示例。
- Gemini 指出：从 `/root/picod` copy binary 不够标准，文档又建议 runtime 用 non-root agent process，最好使用 `/usr/local/bin/picod`。
- 处理判断：用户指出原来的 `agentd` 示例就是 `/usr/local/bin/agentd -> /usr/local/bin/agentd`，所以更专业的修法不是解释 `/root/picod`，而是让 PicoD image layout 也对齐到标准 binary path。
