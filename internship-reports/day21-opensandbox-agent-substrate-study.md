...
- OpenSandbox 社区 issue：[opensandbox-group/OpenSandbox#800](https://github.com/opensandbox-group/OpenSandbox/issues/800)
...
| OpenSandbox | L2 | README 和 Python SDK 明确提供 `opensandbox.Sandbox.create(...)`、`sandbox.commands.run(...)`、`sandbox.files.write_files(...)` / `read_file(...)`、`sandbox.kill()`；README 还说自己定义 `Sandbox Protocol`，提供 OpenAPI specs、CLI、MCP、多语言 SDK；社区 issue [#800](https://github.com/opensandbox-group/OpenSandbox/issues/800) 直接讨论过是否兼容 E2B | 当前看到的是自有 `opensandbox` SDK / OpenSandbox API，不是官方 E2B SDK drop-in；#800 已由维护者关闭为 not planned，理由是 E2B API 是具体 SaaS 产品接口而非开放标准，OpenSandbox 更希望维护自身 `specs/` 下的 lifecycle / execd / egress 公共契约 |
...
OpenSandbox [#800](https://github.com/opensandbox-group/OpenSandbox/issues/800) 尤其值得作为证据记录。issue 提问很直接：是否需要兼容 E2B 接口以方便迁移。维护者回复后关闭为 not planned，核心理由可以概括为：

- E2B API 是具体 SaaS 产品接口，不是开放标准。
- OpenSandbox 希望把自己的 lifecycle API、execd API、egress API 作为稳定公共契约。
- E2B 的 template、envd、process/PTY、beta/deprecated 能力包含较多产品实现细节，完整兼容会明显增加协议和实现复杂度。
- 一旦承诺兼容，就需要长期追齐上游接口变化、错误模型、鉴权、timeout、网络策略、文件操作、端口暴露等细节。
- “接口存在”不等于行为一致，尤其是 pause/resume、PTY、后台命令、文件 watcher、网络访问策略这类边界行为。
- 更现实的方案是迁移文档、对照表、示例，或者可选的 e2b-style wrapper，而不是把 E2B compatibility 放进 OpenSandbox 核心 API。

> 分析：这与我们对 AgentCube 的判断完全一致。AgentCube 如果未来支持 E2B，也不应该把它写进核心 WorkloadManager / Router contract，更不该混入 agent-sandbox 适配 PR。更好的方式是独立 facade：对外模拟 E2B SDK/API 的高频迁移面，对内仍使用 AgentCube 自己的 session lifecycle、Router invocation、Store 和 RuntimeProvider。

> 注释：`e2b-style wrapper` 可以理解成“迁移辅助适配层”：它对外提供类似 E2B 的方法名和最常用语义，但不承诺覆盖 E2B 的全部产品接口，也不把第三方 SaaS 的 API 细节变成项目核心协议。
