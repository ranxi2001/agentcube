...
```text
SDK / CLI / MCP
  -> OpenSandbox Server REST API
     -> Docker runtime
     -> Kubernetes runtime
        -> BatchSandbox provider
        -> agent-sandbox provider
  -> sandbox endpoint
     -> execd inside sandbox
     -> optional ingress / egress / credential vault
```
...
这点和 AgentCube 的 #387 工作直接相关：`agent-sandbox` 上游 CRD 和 lifecycle 逻辑变化会影响使用方。OpenSandbox 的做法值得借鉴的是把 provider 差异收敛在 adapter 层，并在文档里明确 `batchsandbox` 与 `agent-sandbox` 的配置边界。
...
```text
many logical actors
  -> multiplex onto fewer ready workers
  -> suspend idle actors and snapshot state
  -> resume actor on request before routing traffic
```
...
Client request
  -> atenet router / Envoy
     -> query ate-api-server for actor location
        -> if suspended, ResumeActor
           -> scheduler claims a ready Worker
           -> atelet asks ateom-gvisor to restore snapshot
           -> actor becomes RUNNING on worker pod
     -> proxy request to worker pod IP
```
...
OpenSandbox 已经把 `agent-sandbox` 明确当成 provider，而不是把 provider 细节散落到所有业务逻辑中。AgentCube #387 / 后续 v0.5 适配也应继续保持这个思路：
...
3. AgentCube #387 review 时可以引用 OpenSandbox 的 provider adapter 方式，说明 `agent-sandbox` 适配应该集中解释依赖和 CRD 差异。
