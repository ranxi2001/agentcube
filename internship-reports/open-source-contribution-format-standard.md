...
未完成但确实需要 upstream 提前看到的 PR，标题使用 `[WIP] <title>`。不要使用 `[DO NOT MERGE]` 这类自拟标题约定；如果只是为了跑 CI 或验证 bot，先用本地测试、本地 Actions、fork 分支 push 后实际存在的 checks，或 `ci/<topic>` 上的 fork-only push CI；普通 `feat/*` push 没有完整 CI 时要明确记录覆盖缺口。
...
| CI 验证 | 需要验证 CI 但不需要社区 review 时，先跑本地测试 / 本地 Actions，并查看 fork 分支 push 是否产生 checks；当前仓库普通 `feat/*` push 通常没有完整 CI，可在 `ci/<topic>` 验证分支使用 fork-only push CI；不向自己的 fork 仓库提 PR，也不向 upstream 提交临时 PR |
